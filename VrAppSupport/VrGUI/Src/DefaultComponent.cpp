...
OvrDefaultComponent::OvrDefaultComponent( Vector3f const & hilightOffset, float const hilightScale, 
		...
		Vector4f const & textHilightColor, bool const useSurfaceAnimator, const bool noHilight )
...
eMsgStatus OvrDefaultComponent::OnEvent_Impl( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
...
eMsgStatus OvrDefaultComponent::Frame( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
        VRMenuObject * self, VRMenuEvent const & event )
{
    double t = vrFrame.PredictedDisplayTimeInSeconds;
	...
	if ( HilightFader.GetFadeState() == Fader::FADE_NONE && StartFadeInTime <= 0.0 && StartFadeOutTime <= 0.0 )
	{
...
eMsgStatus OvrDefaultComponent::FocusGained( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
        VRMenuObject * self, VRMenuEvent const & event )
{
    ...
    StartFadeOutTime = -1.0;
    StartFadeInTime = FadeDelay + vrFrame.PredictedDisplayTimeInSeconds;
...
eMsgStatus OvrDefaultComponent::FocusLost( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
        VRMenuObject * self, VRMenuEvent const & event )
{
    ...
    StartFadeInTime = -1.0;
    StartFadeOutTime = FadeDelay + vrFrame.PredictedDisplayTimeInSeconds;
...
eMsgStatus OvrSurfaceToggleComponent::OnEvent_Impl( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
	VRMenuObject * self, VRMenuEvent const & event )
{
	switch ( event.EventType )
	{
...
eMsgStatus OvrSurfaceToggleComponent::Frame( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
{
