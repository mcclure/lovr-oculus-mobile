...
bool ovrFileSys::GetPathIfValidPermission( ovrJava const & java, ovrStorageType storageType, ovrFolderType folderType, const char * subfolder, permissionFlags_t permission, std::string & outPath ) 
{
	...
	else 
	{
		...
		ALOGW( "Failed to get permission for %s storage in %s folder ", StorageName[storageType], FolderName[folderType] );
		return false;
	}
...
void ovrFileSys::PushBackSearchPathIfValid( ovrJava const & java, ovrStorageType storageType, ovrFolderType folderType, const char * subfolder, std::vector<std::string> & searchPaths )
{
	std::string storageBasePath = "";
	if ( storageType == EST_SECONDARY_EXTERNAL_STORAGE && folderType == EFT_ROOT )
	{
...
ovrStream *	ovrFileSysLocal::OpenStream( char const * uri, ovrStreamMode const mode )
{
	...
	int port = 0;
	ovrUri::ParseUri( uri, scheme, sizeof( scheme ), nullptr, 0, nullptr, 0, host, sizeof( host ), 
			port, path, sizeof( path ), nullptr, 0, nullptr, 0 );
