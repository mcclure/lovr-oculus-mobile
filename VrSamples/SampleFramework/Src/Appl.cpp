...
*/
	...
	jfieldID asField = env->GetStaticFieldID(contextClass.GetJClass(), "AUDIO_SERVICE", "Ljava/lang/String;");
	if ( 0 == asField )
	{
		ALOGV( "asField = %p ", (void *)asField );
		return;
	}
	jmethodID getSystemServiceID = env->GetMethodID(contextClass.GetJClass(), "getSystemService", "(Ljava/lang/String;)Ljava/lang/Object;");
	if ( 0 == getSystemServiceID )
	{
		ALOGV( "getSystemServiceID = %p ", (void *)getSystemServiceID );
		return;
	}
	jmethodID adjustStreamVolumeID = env->GetMethodID(audioManagerClass.GetJClass(), "adjustStreamVolume", "(III)V");
	...
	/// AudioManager audio = (AudioManager) getSystemService(audioManagerName);
	JavaObject audioManagerObject( env, env->CallObjectMethod( ctx.ActivityObject, getSystemServiceID, audioManagerName.GetJObject() ) );
	...
	/// audio.adjustStreamVolume(AudioManager.STREAM_MUSIC, volumeDelta, 0);
	env->CallVoidMethod(audioManagerObject.GetJObject(), adjustStreamVolumeID, 3 /* STREAM_MUSIC */, volumeDelta, 0 );
}
