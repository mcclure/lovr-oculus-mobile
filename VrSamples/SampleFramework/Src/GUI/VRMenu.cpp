        ...
        menuHandle_t parentHandle_, bool const recenter )
{
	...
	for ( int i = 0; i < static_cast< int >( itemParms.size() ); ++i )
	{
		...
		ALOG( "## Menu[%d] Id=%d Text=%s", i, static_cast< int >( parms->Id.Get() ), parms->Text.c_str() );
#endif
		...
		for ( int j = 0; j < static_cast< int >( itemParms.size() ); ++j )
		{
			...
			{
				ALOG( "Duplicate menu object ids for '%s' and '%s'!",
...
//==============================
// VRMenu::Frame
void VRMenu::Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, Matrix4f const & centerViewMatrix, Matrix4f const & traceMat )
{
	...
	if ( NextMenuState != CurMenuState )
	{
		...
		switch( NextMenuState )
		{
			...
			case MENUSTATE_OPEN:
				{
					assert( CurMenuState != NextMenuState ); // logic below is dependent on this!!
					if ( CurMenuState != MENUSTATE_OPENING ) { ALOG( "Instant open" ); }
			...
			case MENUSTATE_CLOSED:
				{
					assert( CurMenuState != NextMenuState ); // logic below is dependent on this!!
					if ( CurMenuState != MENUSTATE_CLOSING ) { ALOG( "Instant close" ); }
...
//==============================
// VRMenu::OnKeyEvent
bool VRMenu::OnKeyEvent( OvrGuiSys & guiSys, int const keyCode, const int repeatCount )
{
	if ( OnKeyEvent_Impl( guiSys, keyCode, repeatCount ) )
	{
...
/*
	...
	{
		ALOG( "VRMenu '%s' Back key event: %s", GetName(), KeyEventNames[eventType] );
...
{
	ALOG( "VRMenu::Open - '%s', pre - c: %s n: %s", GetName(), MenuStateNames[CurMenuState], MenuStateNames[NextMenuState] );
	...
	ALOG( "VRMenu::Open - %s, post - c: %s n: %s", GetName(), MenuStateNames[CurMenuState], MenuStateNames[NextMenuState] );
}
...
{
	ALOG( "VRMenu::Close - %s, pre - c: %s n: %s", GetName(), MenuStateNames[CurMenuState], MenuStateNames[NextMenuState] );
	...
	ALOG( "VRMenu::Close - %s, post - c: %s n: %s", GetName(), MenuStateNames[CurMenuState], MenuStateNames[NextMenuState] );
}
...
//==============================
// VRMenu::OnItemEvent
void VRMenu::OnItemEvent( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
...
void VRMenu::OnItemEvent_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
		VRMenuId_t const itemId, VRMenuEvent const & event )
{
}
