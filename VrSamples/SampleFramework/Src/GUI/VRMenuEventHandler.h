...
public:
	...
	void			Frame( OvrGuiSys & guiSys, const ovrApplFrameIn & vrFrame,
                            menuHandle_t const & rootHandle, OVR::Posef const & menuPose, OVR::Matrix4f const & traceMat,
							std::vector< VRMenuEvent > & events );

	void			HandleEvents( OvrGuiSys & guiSys, const ovrApplFrameIn & vrFrame,
...
private:
    ...
    bool            DispatchToPath( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
                            VRMenuEvent const & event, std::vector< menuHandle_t > const & path, bool const log ) const;
	bool            BroadcastEvent( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
                            VRMenuEvent const & event, VRMenuObject * receiver ) const;
};
