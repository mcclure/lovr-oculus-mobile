...
void VRMenuMgrLocal::Init( OvrGuiSys & guiSys )
{
	...
	if ( GUIProgramDiffuseOnly.VertexShader == 0 || GUIProgramDiffuseOnly.FragmentShader == 0 )
	{
		...
		GUIProgramDiffuseOnly = OVRFW::GlProgram::Build( GUIDiffuseOnlyVertexShaderSrc, GUIDiffuseOnlyFragmentShaderSrc, uniformParms, uniformCount );
	}
	...
	if ( GUIProgramDiffuseAlphaDiscard.VertexShader == 0 || GUIProgramDiffuseAlphaDiscard.FragmentShader == 0 )
	{
		...
		GUIProgramDiffuseAlphaDiscard = OVRFW::GlProgram::Build( GUIDiffuseOnlyVertexShaderSrc, GUIDiffuseAlphaDiscardFragmentShaderSrc, uniformParms, uniformCount );
	}	
	// diffuse + additive
	if ( GUIProgramDiffusePlusAdditive.VertexShader == 0 || GUIProgramDiffusePlusAdditive.FragmentShader == 0 )
	{
		...
		GUIProgramDiffusePlusAdditive = OVRFW::GlProgram::Build( GUITwoTextureColorModulatedShaderSrc, GUIDiffusePlusAdditiveFragmentShaderSrc, uniformParms, uniformCount );
	}
	// diffuse + diffuse
	if ( GUIProgramDiffuseComposite.VertexShader == 0 || GUIProgramDiffuseComposite.FragmentShader == 0 )
	{
		...
		GUIProgramDiffuseComposite = OVRFW::GlProgram::Build( GUITwoTextureColorModulatedShaderSrc, GUIDiffuseCompositeFragmentShaderSrc, uniformParms, uniformCount );
	}
	...
	if ( GUIProgramDiffuseColorRamp.VertexShader == 0 || GUIProgramDiffuseColorRamp.FragmentShader == 0 )
	{
		...
		GUIProgramDiffuseColorRamp = OVRFW::GlProgram::Build( GUIDiffuseOnlyVertexShaderSrc, GUIColorRampFragmentSrc, uniformParms, uniformCount );
	}
	...
	if ( GUIProgramDiffuseColorRampTarget.VertexShader == 0 || GUIProgramDiffuseColorRampTarget.FragmentShader == 0 )
	{
		...
		GUIProgramDiffuseColorRampTarget = OVRFW::GlProgram::Build( GUIDiffuseColorRampTargetVertexShaderSrc, GUIColorRampTargetFragmentSrc, uniformParms, uniformCount );
	}
	if ( GUIProgramAlphaDiffuse.VertexShader == 0 || GUIProgramAlphaDiffuse.FragmentShader == 0 )
	{
		...
		GUIProgramAlphaDiffuse = OVRFW::GlProgram::Build( GUITwoTextureColorModulatedShaderSrc, GUIAlphaDiffuseFragmentShaderSrc, uniformParms, uniformCount );
	}
...
menuHandle_t VRMenuMgrLocal::CreateObject( VRMenuObjectParms const & parms )
{
	...
	if ( obj == NULL )
	{
		...
		assert( obj != NULL );	// this would be bad -- but we're likely just going to explode elsewhere
		return menuHandle_t();
	}
...
void VRMenuMgrLocal::AppendSurfaceList( Matrix4f const & centerViewMatrix, std::vector< ovrDrawSurface > & surfaceList ) 
{
	if ( NumToRender == 0 )
	{
		return;
