...
public:
    ...
    static OvrGuiSys *		Create( const ovrAppContext * context );
    static void				Destroy( OvrGuiSys * & guiSys );

	virtual void			Init( ovrFileSys * FileSys, SoundEffectPlayer & soundEffectPlayer,
	...
	virtual void			Frame( ovrApplFrameIn const & vrFrame,
									OVR::Matrix4f const & centerViewMatrix ) = 0;

	virtual void			Frame( ovrApplFrameIn const & vrFrame,
									OVR::Matrix4f const & centerViewMatrix, OVR::Matrix4f const & traceMat ) = 0;

	virtual void			AppendSurfaceList( OVR::Matrix4f const & centerViewMatrix, std::vector< ovrDrawSurface > * surfaceList ) const = 0;

	// called when the app menu is up and a key event is received. Return true if the menu consumed
	// the event.
	virtual bool			OnKeyEvent( int const keyCode, const int repeatCount ) = 0;

	virtual void			ResetMenuOrientations( OVR::Matrix4f const & viewMatrix ) = 0;

	virtual HitTestResult	TestRayIntersection( const OVR::Vector3f & start, const OVR::Vector3f & dir ) const = 0;
	...
	virtual	void			ShowInfoText( float const duration, OVR::Vector3f const & offset, OVR::Vector4f const & color, const char * fmt, ... ) = 0;

	//----------------------------------------------------------
	// interfaces

	virtual const ovrAppContext * 	GetContext() = 0;
