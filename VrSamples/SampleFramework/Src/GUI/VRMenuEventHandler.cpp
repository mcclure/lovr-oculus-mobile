...
void VRMenuEventHandler::Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
...
*/
	...
	bool touchPressed = ( vrFrame.AllTouches | ovrTouch_A ) != 0;
	bool touchReleased = !touchPressed && ( ( vrFrame.LastFrameAllButtons | ovrTouch_A ) != 0 ); 
	bool touchDown = ( vrFrame.AllTouches ) != 0;
	bool ignoreTouchRelease = false; /// ( vrFrame.Input.buttonState & BUTTON_TOUCH_WAS_SWIPE ) != 0;

	// Need to update this for keyboard events as well
	for ( int i = 0; i < static_cast<int>( vrFrame.KeyEvents.size() ); i++ )
	{
		if ( vrFrame.KeyEvents[i].KeyCode == AKEYCODE_ENTER )
		{
			switch ( vrFrame.KeyEvents[i].Action )
			{
 ...
 /*   
	...
	int swipeUpDown = ( vrFrame.AllButtons & BUTTON_SWIPE_UP ) ? 2 : 0;
	swipeUpDown = ( vrFrame.AllButtons & BUTTON_SWIPE_DOWN ) ? 1 : swipeUpDown;
	int swipeForwardBack = ( vrFrame.AllButtons & BUTTON_SWIPE_FORWARD ) ? 4 : 0;
	swipeForwardBack = ( vrFrame.AllButtons & BUTTON_SWIPE_BACK ) ? 3 : swipeForwardBack;
...
*/
	...
	Vector2f touchRelative = { 0.0f, 0.0f };
	Vector2f touch = { 0.0f, 0.0f };
	...
	if ( touchReleased )
	{
		...
		{
			VRMenuEvent event( VRMENU_EVENT_SWIPE_COMPLETE, EVENT_DISPATCH_FOCUS, FocusedHandle, Vector3f( touchRelative, 0.0f ), result, "" );
			events.push_back( event );
		}
		else
		{
			VRMenuEvent event( VRMENU_EVENT_TOUCH_UP, EVENT_DISPATCH_FOCUS, FocusedHandle, Vector3f( touchRelative, 0.0f ), result, "" );
			events.push_back( event );
		}		
	}
	if ( touchDown )
	{
		if ( touchRelative.LengthSq() > MATH_FLOAT_SMALLEST_NON_DENORMAL )
		{
			VRMenuEvent event( VRMENU_EVENT_TOUCH_RELATIVE, EVENT_DISPATCH_FOCUS, FocusedHandle, Vector3f( touchRelative, 0.0f ), result, "" );
			events.push_back( event );
		}
		VRMenuEvent event( VRMENU_EVENT_TOUCH_ABSOLUTE, EVENT_DISPATCH_FOCUS, FocusedHandle, Vector3f( touch, 0.0f ), result, "" );
		events.push_back( event );
	}
...
static inline void LogEventType( VRMenuEvent const & event, char const * fmt, ... )
{
	...
	char buffer[512];
	OVR::OVR_sprintf( buffer, sizeof( buffer ), "%s: %s", VRMenuEvent::EventTypeNames[event.EventType], fmtBuff );
...
void VRMenuEventHandler::HandleEvents( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
...
bool VRMenuEventHandler::DispatchToComponents( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
...
bool VRMenuEventHandler::DispatchToPath( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
        VRMenuEvent const & event, std::vector< menuHandle_t > const & path, bool const log ) const
{
	...
	for ( int i = 0; i < static_cast< int >( path.size() ); ++i )
	{
		...
		if ( obj != NULL && DispatchToComponents( guiSys, vrFrame, event, obj ) )
		{
			if ( log )
			{
				ALOG( "%sDispatchToPath: %s, object '%s' consumed event.", &indent[64 - i * 2],
		...
		if ( log )
		{
			ALOG( "%sDispatchToPath: %s, object '%s' passed event.", &indent[64 - i * 2],
					VRMenuEvent::EventTypeNames[event.EventType], obj != NULL ? obj->GetText().c_str() : "<null>" );
		}
	}
	return false;
...
bool VRMenuEventHandler::BroadcastEvent( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
        VRMenuEvent const & event, VRMenuObject * receiver ) const
{
	/// assert_WITH_TAG( receiver != NULL, "VrMenu" );
