...
std::unordered_map< std::string, std::string > RelativeDirectoryFileList( const std::vector< std::string > & searchPaths, const char * RelativeDirPath )
{
	...
	ALOG( "RelativeDirectoryFileList searchPaths=%d relative='%s'", (int)searchPaths.size(), relativeDirPathString.c_str() );
#endif
		...
		if ( datum )
		{
			...
			if ( GetFullPath( searchPaths, s.c_str(), datum->Url ) )
			{
				...
				if ( datumIter == UrlToIndex.end() )
				{
					...
					ALOG( "OvrMetaData adding datum %s with index %d to %s", datum->Url.c_str(), dataIndex, currentCategory.CategoryTag.c_str() );
					// Register with category
					currentCategory.DatumIndicies.push_back( dataIndex );
				}
				else
				{
					ALOGW( "OvrMetaData::InitFromDirectory found duplicate url %s", datum->Url.c_str() );
				}
			}
			else
			{
	...
	for ( int i = 0; i < static_cast< int >( fileList.size() ); ++i )
	{
		...
		if ( datum != NULL )
		{
			...
			if ( datumIter == UrlToIndex.end() )
			{
				...
				UrlToIndex[ lowerCaseUrl ] = dataIndex;
				MetaData.push_back( datum );
				ALOG( "OvrMetaData::InitFromFileList adding datum %s with index %d to %s", datum->Url.c_str(),
...
void OvrMetaData::InsertCategoryList( const int startIndex, const std::vector< Category > & categoryList )
{
	...
	for ( int remoteIndex = 0; remoteIndex < static_cast< int >( categoryList.size() ); ++remoteIndex )
	{
		...
		ALOG( "OvrMetaData::InsertCategoryList merging %s into category index %d", newCategory.CategoryTag.c_str(), targetIndex );
	...
	if ( dataFile != NULL )
	{
		...
		if ( !Categories.empty() )
		{
			...
			for ( Category & cat : Categories )
			{
				...
				{
					ALOGW( "OvrMetaData::ProcessMetaData discarding empty %s", cat.CategoryTag.c_str() );
				}
			}
...
void OvrMetaData::ReconcileCategories( std::vector< Category > & storedCategories )
{
	...
	{
		ALOGW( "OvrMetaData::ReconcileCategories failed to find expected category order -- missing assets/meta.json?" );
	}
	...
	{
		ALOG( "OvrMetaData::ReconcileCategories storedCategory: %s", storedCategory.CategoryTag.c_str() );
	...
	{
		ALOG( "OvrMetaData::ReconcileCategories adding stored category %s", storedCat.CategoryTag.c_str() );
	...
	for ( int metaDataIndex = 0; metaDataIndex < static_cast< int >( MetaData.size() ); ++metaDataIndex )
	{
		...
		for ( const std::string & tag : tags )
		{
			if ( !tag.empty() )
			{
				...
				{
					ALOG( "OvrMetaData inserting index %d for datum %s to %s", metaDataIndex, datum.Url.c_str(), category->CategoryTag.c_str() );
				...
				{
					ALOGW( "OvrMetaData::RegenerateCategoryIndices failed to find category with tag %s for datum %s at index %d",
...
TagAction OvrMetaData::ToggleTag( OvrMetaDatum * metaDatum, const std::string & newTag )
{
	...
	for ( int t = 0; t < static_cast< int >( metaDatum->Tags.size() ); ++t )
	{
		if ( metaDatum->Tags.at( t ) == newTag )
		{
			...
			{
				ALOGW( "ToggleTag attempt to remove only tag: %s on %s", newTag.c_str(), metaDatum->Url.c_str() );
				return TAG_ERROR;
			}
	...
	if ( std::shared_ptr<JSON> data = DataFile->GetItemByName( DATA ) )
	{
		if ( std::shared_ptr<JSON> datum = data->GetItemByIndex( metaDatum->Id ) )
		{
			...
			{
				ALOG( "ToggleTag tag: %s on %s - found node to replace", newTag.c_str(), metaDatum->Url.c_str() );
				datum->ReplaceNodeWith( TAGS, newTagsObject );
				ALOG( "ToggleTag tag: %s on %s - node replaced", newTag.c_str(), metaDatum->Url.c_str() );
				DataFile->Save( FilePath.c_str() );
				ALOG( "ToggleTag tag: %s on %s - file saved", newTag.c_str(), metaDatum->Url.c_str() );
			}
		}
	}
	return action;
...
bool OvrMetaData::GetMetaData( const Category & category, std::vector< const OvrMetaDatum * > & outMetaData ) const
{
	for ( const int metaDataIndex : category.DatumIndicies )
	{
		...
		//ALOG( "Getting MetaData %d title %s from category %s", metaDataIndex, panoData->Title.c_str(), category.CategoryName.c_str() );
...
bool OvrMetaData::ShouldAddFile( const char * filename, const OvrMetaDataFileExtensions & fileExtensions ) const
{
	...
	for ( const std::string& ext : fileExtensions.BadExtensions )
	{
		...
		if ( pathLen > static_cast<size_t>( extLen ) && OVR::OVR_stricmp( filename + pathLen - extLen, ext.c_str() ) == 0 )
		{
			return false;
		}
	}
