...
public:
	...
	void					Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, OVR::Matrix4f const & viewMatrix, OVR::Matrix4f const & traceMat );

	bool					OnKeyEvent( OvrGuiSys & guiSys, int const keyCode, const int repeatCount );
	...
	void					SetMenuPose( OVR::Posef const & pose ) { MenuPose = pose; }
	...
	bool					IsMenu( char const * menuName ) const { return OVR::OVR_stricmp( Name.c_str(), menuName ) == 0; }
...
private:
	...
	virtual void	OnItemEvent_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, 
							VRMenuId_t const itemId, class VRMenuEvent const & event );
