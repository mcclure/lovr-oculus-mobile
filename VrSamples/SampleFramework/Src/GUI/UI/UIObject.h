...
public:
	...
	size_t								GetNumChildren() const { return static_cast< size_t >( Children.size() ); }
	UIObject *							GetChildForIndex( size_t index ) const { return Children[ index ]; }
	...
	OVR::Vector2f const &				GetDimensions() const { return Dimensions; }	// in local scale
	...
	void								SetSurfaceDims( int const surfaceIndex, OVR::Vector2f const &dims );	// requires call to RegenerateSurfaceGeometry() to take effect
