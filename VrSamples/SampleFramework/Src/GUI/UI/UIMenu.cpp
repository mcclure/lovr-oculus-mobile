...
protected:
	...
	bool OnKeyEvent_Impl( OvrGuiSys & guiSys, int const keyCode, const int repeatCount ) override;
private:
	UIMenu * Menu { nullptr };
};
...
bool UIMenuImpl::OnKeyEvent_Impl( OvrGuiSys & guiSys, int const keyCode, const int repeatCount )
{
	assert( Menu );
	if ( Menu->OnKeyEventFunction )
	{
		return Menu->OnKeyEventFunction( Menu, Menu->OnKeyEventFunctionUsrPtr, keyCode, repeatCount );
	}
	return false;
}
