...
namespace OVRFW
{
			...
			const Posef pose, const Vector3f direction )
	{
		...
		GLuint swipeHintTexture = LoadTextureFromUri( guiSys.GetFileSys(), (std::string( "apk:///" ) + img).c_str(), TextureFlags_t( TEXTUREFLAG_NO_DEFAULT ), imgWidth, imgHeight );
	...
	eMsgStatus OvrSwipeHintComponent::OnEvent_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
	...
	eMsgStatus OvrSwipeHintComponent::Opening( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		Reset( self );
		return MSG_STATUS_ALIVE;
	}
	...
	eMsgStatus OvrSwipeHintComponent::Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
