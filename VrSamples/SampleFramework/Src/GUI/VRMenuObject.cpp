...
#include "Appl.h"			// for loading images from the assets folder
...
inline bool Intersect_RayBounds( const Vector3f & rayStart, const Vector3f & rayDir,
		...
		char const * imageName, bool const allowDefault )
{
	...
	if ( imageName != NULL && imageName[0] != '\0' )
	{
		...
		textureHandle_t const h = guiSys.GetTextureManager().LoadTexture( guiSys.GetFileSys(), imageName );
	...
	if ( !Texture.IsValid() && allowDefault )
	{
		...
		Texture = LoadTextureFromBuffer( imageName, MemBuffer( uiDefaultTgaData, uiDefaultTgaSize ),
							TextureFlags_t(), w, h );
#endif
		ALOGW( "VRMenuSurfaceTexture::CreateFromImage: failed to load image '%s' - default loaded instead!", imageName );
	}
...
{
	//assert( SurfaceDef.geo.vertexBuffer == 0 && SurfaceDef.geo.indexBuffer == 0 && SurfaceDef.geo.vertexArrayObject == 0 );
...
void VRMenuSurface::SetTextureSampling( eGUIProgramType const pt )
{
	switch( pt )
	{
		...
		case PROGRAM_DIFFUSE_COLOR_RAMP:			// has a diffuse and color ramp, and color ramp target is the diffuse
		{
			...
			if ( glExtensions.EXT_texture_filter_anisotropic )
			{
                                        ...
                                        eSurfaceTextureType const type, char const * imageName )
{
	...
	{
		/// assert_WITH_TAG( surfaceIndex >= 0 && surfaceIndex < static_cast< int >( Surfaces.size() ), "VrMenu" );
		return;
	}
