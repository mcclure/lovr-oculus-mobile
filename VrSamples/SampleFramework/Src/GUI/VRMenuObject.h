...
public:
	...
	OVR::Vector4f		CropUV;						// specifies the crop range of the texture in UV space, defaults to full texture
...
public:
	...
	OVR::Vector3f				FadeDirection;		// Fades vertices based on direction - default is zero vector which indicates off
	std::string					SurfaceName;		// for debugging only
...
public:
	...
	OVR::Bounds3f const &			GetLocalBounds() const { return Tris.GetBounds(); }

	bool							IsRenderable() const { return ( SurfaceDef.geo.vertexCount > 0 && SurfaceDef.geo.indexCount > 0 ) && Visible; }

    bool							IntersectRay( OVR::Vector3f const & start, OVR::Vector3f const & dir, OVR::Posef const & pose,
									        OVR::Vector3f const & scale, ContentFlags_t const testContents, OvrCollisionResult & result ) const;
    // the ray should already be in local space
    bool							IntersectRay( OVR::Vector3f const & localStart, OVR::Vector3f const & localDir,
									        OVR::Vector3f const & scale, ContentFlags_t const testContents, OvrCollisionResult & result ) const;
	...
	OVR::Vector4f const&			GetColor() const { return Color; }
	void							SetColor( OVR::Vector4f const & color ) { Color = color; }

	OVR::Vector2f const &			GetDims() const { return Dims; }
	void							SetDims( OVR::Vector2f const &dims ) { Dims = dims; } // requires call to CreateFromSurfaceParms or RegenerateSurfaceGeometry() to take effect

	OVR::Vector2f const &			GetAnchors() const { return Anchors; }
	void							SetAnchors( OVR::Vector2f const & a ) { Anchors = a; }

	OVR::Vector2f					GetAnchorOffsets() const;

	OVR::Vector4f const &			GetBorder() const { return Border; }
	void							SetBorder( OVR::Vector4f const & a ) { Border = a; }	// requires call to CreateFromSurfaceParms or RegenerateSurfaceGeometry() to take effect

	OVR::Vector4f const &			GetClipUVs() const { return ClipUVs; }
	void							SetClipUVs( OVR::Vector4f const & uvs ) { ClipUVs = uvs; }

	OVR::Vector2f const &			GetOffsetUVs() const { return OffsetUVs; }
	void							SetOffsetUVs( OVR::Vector2f const & uvs ) { OffsetUVs = uvs; }
...
private:
	void							CreateImageGeometry(  int const textureWidth, int const textureHeight, 
														  const OVR::Vector2f &dims, const OVR::Vector4f &border,
														  const OVR::Vector4f &crop, ContentFlags_t const content );
...
public:
    ...
    bool                IntersectRay( OVR::Vector3f const & localStart, OVR::Vector3f const & localDir, OVR::Vector3f const & parentScale, OVR::Bounds3f const & bounds,
	...
	// always world space.
	menuHandle_t		HitTest( OvrGuiSys const & guiSys, OVR::Posef const & worldPose,
								OVR::Vector3f const & rayStart, OVR::Vector3f const & rayDir,
	...
	OVR::Posef const &	GetLocalPose() const { return LocalPose; }
	void				SetLocalPose( OVR::Posef const & pose ) { LocalPose = pose; }
	OVR::Vector3f const &	GetLocalPosition() const { return LocalPose.Translation; }
	void				SetLocalPosition( OVR::Vector3f const & pos ) { LocalPose.Translation = pos; }
	OVR::Quatf const &	GetLocalRotation() const { return LocalPose.Rotation; }
	void				SetLocalRotation( OVR::Quatf const & rot ) { LocalPose.Rotation = rot; }
	OVR::Vector3f       GetLocalScale() const;
	void				SetLocalScale( OVR::Vector3f const & scale ) { LocalScale = scale; }

    OVR::Posef const &  GetHilightPose() const { return HilightPose; }
    void                SetHilightPose( OVR::Posef const & pose ) { HilightPose = pose; }
    float               GetHilightScale() const { return HilightScale; }
    void                SetHilightScale( float const s ) { HilightScale = s; }

    void                SetTextLocalPose( OVR::Posef const & pose ) { TextLocalPose = pose; }
    OVR::Posef const &  GetTextLocalPose() const { return TextLocalPose; }
    void                SetTextLocalPosition( OVR::Vector3f const & pos ) { TextLocalPose.Translation = pos; }
    OVR::Vector3f const &    GetTextLocalPosition() const { return TextLocalPose.Translation; }
    void                SetTextLocalRotation( OVR::Quatf const & rot ) { TextLocalPose.Rotation = rot; }
    OVR::Quatf const &  GetTextLocalRotation() const { return TextLocalPose.Rotation; }
    OVR::Vector3f       GetTextLocalScale() const;
	float				GetWrapScale() const { return WrapScale; }
    void                SetTextLocalScale( OVR::Vector3f const & scale ) { TextLocalScale = scale; }
	...
	OVR::Bounds3f const &	GetCullBounds() const { return CullBounds; }
	void				SetCullBounds( OVR::Bounds3f const & bounds ) const { CullBounds = bounds; }
    ...
    OVR::Vector4f const &	GetTextColor() const { return TextColor; }
    void				SetTextColor( OVR::Vector4f const & c ) { TextColor = c; }
	...
	OVR::Vector3f const &	GetFadeDirection() const { return FadeDirection;  }
	void				SetFadeDirection( OVR::Vector3f const & dir ) { FadeDirection = dir;  }
	...
	//--------------------------------------------------------------
	void							GetWorldTransform( OvrVRMenuMgr & menuMgr, OVR::Posef const & menuPose,
											OVR::Posef & outPose, OVR::Vector3f & outScale, OVR::Vector4f & outColor ) const;

	static void						TransformByParent( OVR::Posef const & parentPose,
											OVR::Vector3f const & parentScale, OVR::Vector4f const & parentColor,
											OVR::Posef const & localPose, OVR::Vector3f const & localScale, OVR::Vector4f const & localColor,
											VRMenuObjectFlags_t const & objectFlags,
											OVR::Posef & outPose, OVR::Vector3f & outScale, OVR::Vector4f & outColor );
...
private:
    ...
    OVR::Posef                  TextLocalPose;  // local-space position and orientation of text, local to this node (i.e. after LocalPose / LocalScale are applied)
	...
	mutable OVR::Bounds3f		CullBounds;			// bounds of this object and all its children in the local space of its parent
...
private:
	...
	bool						IntersectRayBounds( OVR::Vector3f const & start, OVR::Vector3f const & dir,
										OVR::Vector3f const & mins, OVR::Vector3f const & maxs,
										ContentFlags_t const testContents, float & t0, float & t1 ) const;

	// Test the ray against this object and all child objects, returning the first object that was
	// hit by the ray. The ray should be in parent-local space - for the current root menu this is
	// always world space.
	bool						HitTest_r( OvrGuiSys const & guiSys, OVR::Posef const & parentPose, OVR::Vector3f const & parentScale,
                                        OVR::Vector3f const & rayStart, OVR::Vector3f const & rayDir,  ContentFlags_t const testContents,
                                        HitTestResult & result ) const;
