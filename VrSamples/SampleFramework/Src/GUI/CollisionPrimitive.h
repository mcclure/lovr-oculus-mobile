...
public:
	...
	virtual  bool		IntersectRay( OVR::Vector3f const & start, OVR::Vector3f const & dir, OVR::Posef const & pose,
								OVR::Vector3f const & scale, ContentFlags_t const testContents,
								OvrCollisionResult & result ) const = 0;
	// the ray should already be in local space
	virtual bool		IntersectRay( OVR::Vector3f const & localStart, OVR::Vector3f const & localDir,
								OVR::Vector3f const & scale, ContentFlags_t const testContents,
								OvrCollisionResult & result ) const = 0;
	...
	virtual void		DebugRender( OvrDebugLines & debugLines, OVR::Posef & pose, OVR::Vector3f const & scale, 
	...
	OVR::Bounds3f const &	GetBounds() const { return Bounds; }
	void				SetBounds( OVR::Bounds3f const & bounds ) { Bounds = bounds; }

protected:
	bool				IntersectRayBounds( OVR::Vector3f const & start, OVR::Vector3f const & dir, 
							    OVR::Vector3f const & scale, float & t0, float & t1 ) const;
...
public:
	OvrTriCollisionPrimitive();
	OvrTriCollisionPrimitive( std::vector< OVR::Vector3f > const & vertices, std::vector< TriangleIndex > const & indices, 
			std::vector< OVR::Vector2f > const & uvs, ContentFlags_t const contents );

	virtual	~OvrTriCollisionPrimitive();

	void				Init( std::vector< OVR::Vector3f > const & vertices, std::vector< TriangleIndex > const & indices,
								std::vector< OVR::Vector2f > const & uvs, ContentFlags_t const contents );
