...
public:
	...
	void					InitFromFileListMergeMeta( const std::vector< std::string > & fileList, const std::vector< std::string > & searchPaths,
		const OvrMetaDataFileExtensions & fileExtensions, const char * appFileStoragePath, const char * metaFile, std::shared_ptr<OVR::JSON> storedMetaData );
	...
	void					ProcessMetaData( std::shared_ptr<OVR::JSON> dataFile, const std::vector< std::string > & searchPaths, const char * metaFile );
	...
	std::shared_ptr<OVR::JSON>	CreateOrGetStoredMetaFile( const char * appFileStoragePath, const char * metaFile );
...
protected:
	...
	virtual	void			ExtractExtendedData( const OVR::JsonReader & jsonDatum, OvrMetaDatum & outDatum ) const = 0;
	virtual	void			ExtendedDataToJson( const OvrMetaDatum & datum, std::shared_ptr<OVR::JSON> outDatumObject ) const = 0;
	...
	void					ExtractCategories( std::shared_ptr<OVR::JSON> dataFile, std::vector< Category > & outCategories ) const;
	void					ExtractMetaData( std::shared_ptr<OVR::JSON> dataFile, const std::vector< std::string > & searchPaths, std::unordered_map< std::string, OvrMetaDatum * > & outMetaData ) const;
	void					ExtractRemoteMetaData( std::shared_ptr<OVR::JSON> dataFile, std::unordered_map< std::string, OvrMetaDatum * > & outMetaData ) const;
	void					Serialize();
