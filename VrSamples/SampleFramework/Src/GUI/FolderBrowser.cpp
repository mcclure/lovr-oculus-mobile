...
private:
										...
										VRMenuObject * self, VRMenuEvent const & event )
	{
		switch( event.EventType )
        {
            ...
            default:
                assert( !"Event flags mismatch!" ); // the constructor is specifying a flag that's not handled
                return MSG_STATUS_ALIVE;
        }
	}

	eMsgStatus Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		if ( ValidFoldersCount > 1 ) // Need at least one folder in order to enable vertical scrolling
		{
			...
			if ( controllerInput & ( ovrButton_Up | ovrButton_Down | ovrButton_Left | ovrButton_Right ) )
			{
	...
	eMsgStatus TouchDown( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
	...
	eMsgStatus TouchUp( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		if ( FolderBrowser.HasNoMedia() )
		{
			return MSG_STATUS_CONSUMED;
	...
	eMsgStatus TouchRelative( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		if ( FolderBrowser.HasNoMedia() )
		{
			return MSG_STATUS_CONSUMED;
	...
	eMsgStatus OnOpening( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		return MSG_STATUS_ALIVE;
	}
...
private:
    ...
    virtual eMsgStatus Frame( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
    {
    	assert( FolderPtr );
    ...
    eMsgStatus OnEvent_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
    {
    	switch( event.EventType )
    	{
    		case VRMENU_EVENT_FRAME_UPDATE:
	...
	eMsgStatus OnTouchDown_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		ScrollMgr.TouchDown();
		TouchDown = true;
		return MSG_STATUS_ALIVE;
	...
	eMsgStatus OnTouchUp_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		ScrollMgr.TouchUp();
		TouchDown = false;
		return MSG_STATUS_ALIVE;
	...
	, TouchDirectionLocked( NO_LOCK )
{
	...
	// load up the default panel textures once
	{
		...
		for ( int t = 0; t < 2; ++t )
		{
			int width = 0;
			int height = 0;
			DefaultPanelTextureIds[ t ] = LoadTextureFromUri( guiSys.GetFileSys(), panelSrc[ t ], TextureFlags_t( TEXTUREFLAG_NO_DEFAULT ), width, height );
...
void OvrFolderBrowser::OneTimeInit( OvrGuiSys & guiSys )
{
	...
	ovrFileSys::GetPathIfValidPermission( java, EST_PRIMARY_EXTERNAL_STORAGE, EFT_CACHE, "", permissionFlags_t( PERMISSION_WRITE ), AppCachePath );
	assert( !AppCachePath.empty() );

	ovrFileSys::PushBackSearchPathIfValid( java, EST_SECONDARY_EXTERNAL_STORAGE, EFT_ROOT, "RetailMedia/", ThumbSearchPaths );
	ovrFileSys::PushBackSearchPathIfValid( java, EST_SECONDARY_EXTERNAL_STORAGE, EFT_ROOT, "", ThumbSearchPaths );
	ovrFileSys::PushBackSearchPathIfValid( java, EST_PRIMARY_EXTERNAL_STORAGE, EFT_ROOT, "RetailMedia/", ThumbSearchPaths );
	ovrFileSys::PushBackSearchPathIfValid( java, EST_PRIMARY_EXTERNAL_STORAGE, EFT_ROOT, "", ThumbSearchPaths );
			...
			else // already have this folder - rebuild it with the new metadata
			{
				...
				for ( ; existingIndex < numCategories; ++existingIndex )
				{
					...
					if ( cat.CategoryTag == currentCategory.CategoryTag )
					{
						...
						{
							ALOG( "Failed to get any metaData for folder %i named %s", existingIndex, currentCategory.CategoryTag.c_str() );
						}
						break;
					}
				}
...
void OvrFolderBrowser::BuildFolderView( OvrGuiSys & guiSys, OvrMetaData::Category & category, FolderView * const folder, const OvrMetaData & metaData, VRMenuId_t foldersRootId, int folderIndex )
{
	...
	ALOG( "Building Folder %s id: %lld with %d panels", category.CategoryTag.c_str(), folderId.Get(), numPanels );
	std::vector< VRMenuComponent* > comps;
...
void OvrFolderBrowser::RebuildFolderView( OvrGuiSys & guiSys, OvrMetaData & metaData, const int folderIndex, const std::vector< const OvrMetaDatum * > & data )
{
	if ( folderIndex >= 0 && static_cast< int >( Folders.size() ) > folderIndex )
	{
		...
		if ( folder == NULL )
		{
			ALOG( "OvrFolderBrowser::RebuildFolder failed to Folder for folderIndex %d", folderIndex );
			return;
		}
...
void OvrFolderBrowser::LoadThumbnailToTexture( OvrGuiSys & guiSys, const char * thumbnailCommand )
{
	...
	{
		ALOGW( "OvrFolderBrowser::LoadThumbnailToTexture failed to find panel id %d in folder %d", panelId, folderId );
		free( data );
		return;
	}
	...
	{
		ALOGW( "OvrFolderBrowser::LoadThumbnailToTexture Failed to apply AA to %s", thumbnailCommand );
	}
...
void OvrFolderBrowser::QueueAsyncThumbnailLoad( const OvrMetaDatum * panoData, const int folderIndex, const int panelId )
{
	...
	{
		ALOGW( "OvrFolderBrowser::QueueAsyncThumbnailLoad error invalid folder index: %d", folderIndex );
		return;
	}
	...
	OVR::OVR_sprintf( appCacheThumbPath, sizeof( appCacheThumbPath ), "%s%s", AppCachePath.c_str(), ThumbName( relativeThumbPath ).c_str() );
