...
public:
	...
	eMsgStatus FocusGained( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event );

	eMsgStatus FocusLost( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame, VRMenuObject * self, VRMenuEvent const & event );

	static OVR::Vector3f CalcIconFadeOffset( char const * text, BitmapFont const & font, OVR::Vector3f const & axis, float const iconWidth );
