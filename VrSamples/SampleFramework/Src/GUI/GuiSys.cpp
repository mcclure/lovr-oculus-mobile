...
{
public:
							OvrGuiSysLocal( const ovrAppContext * context );
	virtual					~OvrGuiSysLocal();

	virtual void			Init( ovrFileSys * FileSys, OvrGuiSys::SoundEffectPlayer & soundEffectPlayer,
							        char const * fontName, OvrDebugLines * debugLines ) override;
	...
	virtual void			Frame( ovrApplFrameIn const & vrFrame,
									Matrix4f const & viewMatrix ) override;
	virtual void			Frame( ovrApplFrameIn const & vrFrame,
									Matrix4f const & viewMatrix, Matrix4f const & traceMat ) override;

	virtual void 			AppendSurfaceList( Matrix4f const & centerViewMatrix, std::vector< ovrDrawSurface > * surfaceList ) const override;

	virtual bool			OnKeyEvent( int const keyCode, const int repeatCount ) override;

	virtual void			ResetMenuOrientations( Matrix4f const & viewMatrix ) override;

	virtual HitTestResult	TestRayIntersection( const Vector3f & start, const Vector3f & dir ) const override;
	...
	virtual	void			ShowInfoText( float const duration, Vector3f const & offset, Vector4f const & color, const char * fmt, ... ) override;

	virtual const ovrAppContext * 	GetContext() override { return Context; }
	virtual ovrFileSys &			GetFileSys() override { return *FileSys; }
	virtual OvrVRMenuMgr &			GetVRMenuMgr() override { return *MenuMgr; }
	virtual OvrVRMenuMgr const &	GetVRMenuMgr() const override { return *MenuMgr; };
	virtual OvrGazeCursor &			GetGazeCursor() override { return *GazeCursor; }
	virtual BitmapFont &			GetDefaultFont() override { return *DefaultFont; }
	virtual BitmapFont const &		GetDefaultFont() const override { return *DefaultFont; }
	virtual BitmapFontSurface &		GetDefaultFontSurface() override { return *DefaultFontSurface; }
	virtual OvrDebugLines &			GetDebugLines() override { return *DebugLines; }
	virtual SoundEffectPlayer &		GetSoundEffectPlayer() override { return *SoundEffectPlayer; }
	virtual ovrTextureManager &		GetTextureManager() override { return *TextureManager; } // app's texture manager should always initialize before guisys

	virtual ovrReflection &			GetReflection() override { return *Reflection; }
	virtual ovrReflection const &	GetReflection() const override { return *Reflection; }


private:
	const ovrAppContext * 	Context;
	ovrFileSys *			FileSys;
...
//==============================
// OvrGuiSys::Create
OvrGuiSys * OvrGuiSys::Create( const ovrAppContext * context )
{
...
//==============================
// OvrGuiSysLocal::
OvrGuiSysLocal::OvrGuiSysLocal( const ovrAppContext * context )
	: Context( context )
	, FileSys( nullptr )
	, MenuMgr( nullptr )
	, GazeCursor( nullptr )
...
//==============================
// OvrGuiSysLocal::Init
void OvrGuiSysLocal::Init( ovrFileSys * FileSys, OvrGuiSys::SoundEffectPlayer & soundEffectPlayer, char const * fontName,
		BitmapFontSurface * fontSurface, OvrDebugLines * debugLines )
{
	...
	assert( fontSurface->IsInitialized() );	// if you pass a font surface in, you must initialized it before calling OvrGuiSysLocal::Init()
...
//==============================
// OvrGuiSysLocal::Init
void OvrGuiSysLocal::Init( ovrFileSys * FileSys, OvrGuiSys::SoundEffectPlayer & soundEffectPlayer, char const * fontName, OvrDebugLines * debugLines )
{
	BitmapFontSurface * fontSurface = BitmapFontSurface::Create();
	fontSurface->Init( 8192 );
	Init( FileSys, soundEffectPlayer, fontName, fontSurface, debugLines );
}
...
//==============================
// OvrGuiSysLocal::Frame
void OvrGuiSysLocal::Frame( const ovrApplFrameIn & vrFrame, Matrix4f const & centerViewMatrix, Matrix4f const & traceMat )
{
	...
	/// TODO - validate this
	Matrix4f lastViewMatrix = vrapi_GetTransformFromPose( (ovrPosef *)&vrFrame.Tracking.HeadPose );  
...
bool OvrGuiSysLocal::OnKeyEvent( int const keyCode, const int repeatCount )
{
	if ( !IsInitialized )
	{
		assert( IsInitialized );
	...
	for ( int i = 0; i < static_cast< int >( ActiveMenus.size() ); ++i )
	{
		...
		{
			ALOG( "OvrGuiSysLocal back key event '%s' for menu '%s'", KeyEventNames[eventType], curMenu->GetName() );
		}
#endif
		if ( curMenu->OnKeyEvent( *this, keyCode, repeatCount ) )
		{
