...
inline float LinearRangeMapFloat( float inValue, float inStart, float inEnd, float outStart, float outEnd )
{
	...
	const int numElements, const bool verticalBar, const Vector4f & thumbBorder, std::vector< const VRMenuObjectParms* > & parms )
{
	...
	// add parms for the base image that underlays the whole scrollbar
	{
		...
		GLuint sbTexture = LoadTextureFromUri( guiSys.GetFileSys(), GetImage( SCROLLBAR_IMAGE_BASE, verticalBar ).c_str(), TextureFlags_t( TEXTUREFLAG_NO_DEFAULT ), sbWidth, sbHeight );
		if ( verticalBar )
		{
	...
	// add parms for the thumb image of the scrollbar
	{
		...
		GLuint stTexture = LoadTextureFromUri( guiSys.GetFileSys(), GetImage( SCROLLBAR_IMAGE_THUMB, verticalBar ).c_str(), TextureFlags_t( TEXTUREFLAG_NO_DEFAULT ), stWidth, stHeight );
...
eMsgStatus OvrScrollBarComponent::OnEvent_Impl( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
...
eMsgStatus OvrScrollBarComponent::OnFrameUpdate( OvrGuiSys & guiSys, ovrApplFrameIn const & vrFrame,
		VRMenuObject * self, VRMenuEvent const & event )
{
	assert( self != NULL );
