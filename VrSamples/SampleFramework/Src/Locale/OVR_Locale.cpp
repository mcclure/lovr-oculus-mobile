...
public:
	...
	virtual bool			GetLocalizedString( char const * key, char const * defaultStr, std::string & out ) const;
...
bool ovrLocaleInternal::GetStringJNI( char const * key, char const * defaultOut, std::string & out ) const
{
	...
	{
		ALOG( "GetStringJNI activityClass == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jmethodID getPackageNameMethod = jni.GetMethodID( activityClass, "getPackageName", "()Ljava/lang/String;"  );
	if ( getPackageNameMethod == NULL )
	{
		ALOG( "GetStringJNI getPackageNameMethod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jstring packageName = (jstring) jni.CallObjectMethod( activityObject, getPackageNameMethod ); 
	if ( packageName == NULL )
	{
		ALOG( "GetStringJNI packageName == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jmethodID getResourcesMethod = jni.GetMethodID( activityClass, "getResources", "()Landroid/content/res/Resources;"  );
	if ( getResourcesMethod == NULL )
	{
		ALOG( "GetStringJNI getResourcesMethod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jobject resources = (jstring) jni.CallObjectMethod( activityObject, getResourcesMethod ); 
	if ( resources == NULL )
	{
		ALOG( "GetStringJNI resources == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jclass resourcesClass = jni.GetObjectClass( resources ); 
	if ( resources == NULL )
	{
		ALOG( "GetStringJNI resources == NULL" );
		out = "JAVAERROR";
	...
	jmethodID getIdentifierMethod = jni.GetMethodID( resourcesClass, "getIdentifier", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I" );
	if ( getIdentifierMethod == NULL )
	{
		ALOG( "GetStringJNI getIdentifierMethod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jobject keyObj = jni.NewStringUTF( realKey );
	if ( keyObj == NULL )
	{
		ALOG( "GetStringJNI keyObj == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jobject stringObj = jni.NewStringUTF( "string" );
	if ( stringObj == NULL )
	{
		ALOG( "GetStringJNI stringObj == NULL" );
		out = "JAVAERROR";
		return false;
	}

	int id = jni.CallIntMethod( resources, getIdentifierMethod, keyObj, stringObj, packageName ); 
	if ( id == 0 )
	{
	...
	jmethodID getTextMethod = jni.GetMethodID( resourcesClass, "getText", "(Ljava/lang/Integer;)Ljava/lang/String;" );
	if ( getTextMethod == NULL )
	{
		ALOG( "GetStringJNI getTextMethod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jobject textObject = jni.CallObjectMethod( resources, getTextMethod, id ); 
	if ( textObject == NULL )
	{
		ALOG( "GetStringJNI textObject == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jclass textObjectClass = jni.GetObjectClass( textObject ); 
	if ( textObjectClass == NULL )
	{
		ALOG( "GetStringJNI textObjectClass == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jmethodID toStringMethdod = jni.GetMethodID( textObjectClass, "toString", "()Ljava/lang/String;" );
	if ( toStringMethdod == NULL )
	{
		ALOG( "GetStringJNI toStringMethdod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	jstring textObjectString = (jstring) jni.CallObjectMethod( textObject, toStringMethdod );
	if ( textObjectString == NULL )
	{
	...
	const char * textObjectString_ch = jni.GetStringUTFChars( textObjectString, 0 );
	out = textObjectString_ch;
	jni.ReleaseStringUTFChars( textObjectString, textObjectString_ch );
	return true;
}
...
bool ovrLocaleInternal::GetLocalizedString( char const * key, char const * defaultStr, std::string & out ) const
{
	if ( key == NULL )
	{
		return false;
...
void ovrLocaleInternal::ReplaceLocalizedText( char const * inText, char * out, size_t const outSize ) const
{
	...
	while( cur != nullptr )
	{
		...
		if ( !CopyChars( out, outSize, outOfs, localized.c_str(), OVR::OVR_strlen( localized.c_str() ) ) )
		{
			return;
		}
...
ovrLocale * ovrLocale::Create( JNIEnv & jni_, jobject activity_, char const * name, ovrFileSys * fileSys )
{
	...
	{
		jmethodID getDefaultMethod = jni_.GetStaticMethodID( localeClass, "getDefault", "()Ljava/util/Locale;" );
		...
		else
		{
			...
			{
				jmethodID getLanguageMethod = jni_.GetMethodID( localeClass, "getLanguage", "()Ljava/lang/String;"  );
				...
				{
					jstring language = (jstring) jni_.CallObjectMethod( defaultLocale, getLanguageMethod ); 
					if ( language )
					{
