...
struct ovrApplFrameIn
{
	...
	ovrInputStateHeadset		Headset;
	uint16_t					SingleHandRemoteTrackpadMaxX = 0u;
	uint16_t					SingleHandRemoteTrackpadMaxY = 0u;
	uint16_t					HeadsetTrackpadMaxX = 0u;
	uint16_t					HeadsetTrackpadMaxY = 0u;
	...
	bool						HeadsetInputTracked = false;
...
public:
	...
	explicit ovrApplFrameOut( const bool exitApp )
		: ExitApp( exitApp )
	{
	}
...
public:
	...
	ovrAppl(  const int32_t mainThreadTid, const int32_t renderThreadTid,
		const int cpuLevel, const int gpuLevel, bool useMutliView = true )
		: CpuLevel( cpuLevel )
		, GpuLevel( gpuLevel )
		, MainThreadTid( mainThreadTid )
		, RenderThreadTid( renderThreadTid )
		, UseMultiView( useMutliView )
		, NumFramebuffers( useMutliView ? 1 : VRAPI_FRAME_LAYER_EYE_MAX )
	{
	}
