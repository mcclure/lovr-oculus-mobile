...
ovrLexer::ovrLexer( const char * source )
	: ovrLexer( source, OVR::OVR_strlen( source ), NULL )
{
}
...
{
	bool result = ch == '\"';
	assert( result == false || ch <= 255 );	// quotes are assumed elsewhere to be single-byte characters!
	return result;
}
...
void ovrLexer::CopyResult( char const * buffer, char * token, size_t const maxTokenSize )
{
	...
	if ( IsQuote( *buffer ) )
	{
		...
		if ( IsQuote( lastChar ) )
		{
			...
			len = OVR::OVR_strlen( start );	// We do not care about UTF length here since we know the quotes are a single bytes
