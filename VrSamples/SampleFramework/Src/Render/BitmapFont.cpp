...
bool AppendUriPath( char const * inPath, char const * appendPath, char * outPath, size_t const outPathSize )
{
	const char WIN_PATH_SEPARATOR = '\\';
	const char NIX_PATH_SEPARATOR = '/';
	const char URI_PATH_SEPARATOR = '/';
...
#if defined( OVR_BUILD_DEBUG )
///	ALOG( "BitmapFontSurfaceLocal::DrawText3D( \"%s\" %s %s ) : width = %.2f, height = %.2f, numLines = %i, fh = %.2f",
///			text, 
...
///			width, height, numLines, AsLocal( font ).GetFontInfo().FontHeight );
	...
	{
		ALOG( "DrawTextToVertexBlock: normal = ( %g, %g, %g ), text = '%s'", normal.x, normal.y, normal.z, text );
...
bool FontInfoType::LoadFromBuffer( void const * buffer, size_t const bufferSize )
{
	...
	if ( jsonRoot == NULL )
	{
		...
		ALOG( "FontInfoType::LoadFromBuffer FAIL OVR::JSON ERROR = '%s' ", ( errorMsg != NULL ) ? errorMsg : "<NULL>" );
		return false;
	}
	...
	// we scale everything after loading integer values from the OVR::JSON file because the OVR OVR::JSON writer loses precision on floats
...
#if defined( OVR_BUILD_DEBUG )
///				ALOG( "FontWeights[%d] --> AlphaCenterOffset=%.3f ColorCenterOffset=%.3f", i, w.AlphaCenterOffset, w.ColorCenterOffset );
#endif
			}
		}
	}
...
bool FontInfoType::Save( char const * path )
{
	...
	for ( const int & index : glyphIndices )
	{
		...
		joGlyph->AddNumberItem( "X", g.X );	// now in natural units because the OVR::JSON writer loses precision
		joGlyph->AddNumberItem( "Y", g.Y );		// now in natural units because the OVR::JSON writer loses precision
...
void BitmapFontSurfaceLocal::Finish( Matrix4f const & viewMatrix )
{
	...
	for ( int i = 0; i < static_cast< int >( VertexBlocks.size() ); ++i )
	{
		...
		for ( int j = 0; j < vb.NumVerts; j++ )
		{
			...
			*(std::uint32_t*)(&Vertices[CurVertex].fontParms[0]) = *(std::uint32_t*)(&v.fontParms[0]);
			CurVertex++;
