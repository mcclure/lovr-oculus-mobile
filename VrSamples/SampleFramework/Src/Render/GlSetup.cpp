...
{
	ALOG( "GL_Setup: requestGlEsVersion(%d), redBits(%d), greenBits(%d), blueBits(%d), depthBits(%d), multisamples(%d), contextPriority(%d)",
	...
	{
		ALOG( "Trying for a EGL_CONTEXT_CLIENT_VERSION %i context shared with %p:",
		...
		if ( egl.context != EGL_NO_CONTEXT )
		{
			...
			ALOG( "Created context with config %p, query returned ID %i = config %p",
	...
	if ( contextPriority != EGL_CONTEXT_PRIORITY_MEDIUM_IMG )
	{
		...
		{
			case EGL_CONTEXT_PRIORITY_HIGH_IMG:		ALOG( "Context is EGL_CONTEXT_PRIORITY_HIGH_IMG" ); break;
			case EGL_CONTEXT_PRIORITY_MEDIUM_IMG:	ALOG( "Context is EGL_CONTEXT_PRIORITY_MEDIUM_IMG" ); break;
			case EGL_CONTEXT_PRIORITY_LOW_IMG:		ALOG( "Context is EGL_CONTEXT_PRIORITY_LOW_IMG" ); break;
			default:								ALOG( "Context has unknown priority level" ); break;
		}
	}
