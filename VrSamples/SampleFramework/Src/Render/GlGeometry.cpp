...
GlGeometry BuildTesselatedCylinder( const float radius, const float height, const TriangleIndex horizontal, const TriangleIndex vertical, const float uScale, const float vScale )
{
	...
	for ( int y = 0; y <= vertical; ++y )
	{
		...
		for ( int x = 0; x <= horizontal; ++x )
		{
			...
			attribs.normal[index] = Vector3f(attribs.position[index].x, attribs.position[index].y, 0).Normalized();
...
GlGeometry BuildTesselatedCapsule( const float radius, const float height, const TriangleIndex horizontal, const TriangleIndex vertical )
{
	const int vertexCount = ( horizontal + 1 ) * ( vertical + 1 );
	...
	/// Upper Dome
	{
		for ( int y = 0; y <= vertical; y++ )
		{
			...
			for ( int x = 0; x <= horizontal; x++ )
			{
				...
				attribs.normal[index] = Vector3f(attribs.position[index].x, attribs.position[index].y, attribs.position[index].z -h).Normalized();
			}
		}
...
GlGeometry BuildUnitCube( float side )
{
	VertexAttribs attribs;

	// positions
	attribs.position = {
		{ -side, +side, -side }, { +side, +side, -side }, { +side, +side, +side }, { -side, +side, +side },	// top
		{ -side, -side, -side }, { -side, -side, +side }, { +side, -side, +side }, { +side, -side, -side },	// bottom
		{ +side, -side, -side }, { +side, +side, -side }, { +side, +side, +side }, { +side, -side, +side },	// right 
		{ -side, -side, -side }, { -side, -side, +side }, { -side, +side, +side }, { -side, +side, -side }, // left
		{ -side, -side, +side }, { +side, -side, +side }, { +side, +side, +side }, { -side, +side, +side },	// front
		{ -side, -side, -side }, { -side, +side, -side }, { +side, +side, -side }, { +side, -side, -side },	// back
	};
	attribs.normal = {
