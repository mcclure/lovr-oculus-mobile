...
void ovrPanelRenderer::Init()
{
    Program = GlProgram::Build(
    	...
    	FragmentSrc,
		UniformParms,
		sizeof( UniformParms ) / sizeof( UniformParms[0] ) );

	SurfaceDef.geo = BuildTesselatedQuad( 1, 1, false );

	ChannelDataBuffer.Create( GLBUFFER_TYPE_UNIFORM, NUM_DATA_POINTS * sizeof( Vector4f ), UniformChannelData.data() );
...
void ovrPanelRenderer::Update( const OVR::Vector4f & dataPoint )
{
	...
	ChannelDataBuffer.Update( UniformChannelData.size() * sizeof( Vector4f ), UniformChannelData.data() );
}
