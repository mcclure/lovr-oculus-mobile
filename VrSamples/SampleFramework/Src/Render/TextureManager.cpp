		...
		ovrTextureFilter const filterType, ovrTextureWrap const wrapType )
{
	...
	if ( !tex.IsValid() )
	{
		ALOG( "LoadTextureFromBuffer( '%s', %p, %u ) failed!", uri, buffer, static_cast< uint32_t >( bufferSize ) );
		return textureHandle_t();
	}
		...
		ovrTextureFilter const filterType, ovrTextureWrap const wrapType )
{
	...
	GlTexture tex;
	{
		...
		if ( !tex.IsValid() )
		{
			//ALOG( "LoadRGBATextureFromMemory( '%s', %p, %i, %i ) failed!", uri, imageData, imageWidth, imageHeight );
			return textureHandle_t();
		}
	}
		...
		ovrTextureFilter const filterType, ovrTextureWrap const wrapType )
{
	...
	GlTexture tex;
	{
		...
		if ( !tex.IsValid() )
		{
			//ALOG( "LoadRGBATextureFromMemory( %d, %p, %i, %i ) failed!", iconId, imageData, imageWidth, imageHeight );
			return textureHandle_t();
		}
	}
