...
public:
	...
	virtual bool 					IsEmpty() const override { return Points.size() == 0; }
	virtual bool 					IsFull() const override { return false; }
	virtual int 					GetFirst() const override { return 0; }
	virtual int						GetNext( const int cur ) const override;
	virtual int						GetLast() const override { return (int)Points.size() - 1; }
	virtual int 					GetCurPoints() const override { return Points.size(); }
	virtual int 					GetMaxPoints() const override { return MaxPoints; }

	virtual const OVR::Vector3f & 	Get( const int index ) const override { return Points[index]; }
	virtual OVR::Vector3f & 		Get( const int index ) override { return Points[index]; }

	virtual void  					AddToTail( const OVR::Vector3f & p ) override { Points.push_back( p ); }
...
public:
	...
	virtual bool IsEmpty() const override { return HeadIndex == TailIndex; 	}
	virtual bool IsFull() const override { return IncIndex( TailIndex ) == HeadIndex; }
	virtual int GetFirst() const override;
	virtual int GetNext( const int cur ) const override;
	virtual int GetLast() const override;
	virtual int GetCurPoints() const override { return CurPoints; }
	virtual int GetMaxPoints() const override { return BufferSize - 1; 	}
