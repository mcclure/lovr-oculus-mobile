...
public:
	...
	void				Frame( const OVRFW::ovrApplFrameIn & frame, const ovrTextureAtlas & textureAtlas, 
								const OVR::Matrix4f & centerEyeViewMatrix );

	void				Shutdown();

	void				RenderEyeView( OVR::Matrix4f const & viewMatrix, OVR::Matrix4f const & projectionMatrix,
								std::vector< ovrDrawSurface > & surfaceList ) const;

	handle_t		 	AddParticle( const OVRFW::ovrApplFrameIn & frame, 
								const OVR::Vector3f & initialPosition, const float initialOrientation, 
								const OVR::Vector3f & initialVelocity, const OVR::Vector3f & acceleration, 
								const OVR::Vector4f & initialColor, const ovrEaseFunc easeFunc, const float rotationRate, 
								const float scale, const float lifeTime, const uint16_t spriteIndex );
	
	void				UpdateParticle( const OVRFW::ovrApplFrameIn & frame, const handle_t handle,
								const OVR::Vector3f & position, const float orientation, 
								const OVR::Vector3f & velocity, const OVR::Vector3f & acceleration, 
								const OVR::Vector4f & color, const ovrEaseFunc easeFunc, const float rotationRate, 
								const float scale, const float lifeTime, const uint16_t spriteIndex );
