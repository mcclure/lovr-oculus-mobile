...
bool GLCheckErrorsWithTitle( const char * logTitle )
{
	...
	do
	{
		...
		hadError = true;
		ALOGW( "%s GL Error: #0x%04x -> %s", ( logTitle != NULL ) ? logTitle : "<untitled>", (int)err, EglErrorString( err ) );
...
void GL_InvalidateFramebuffer( const enum invalidateTarget_t isFBO, const bool colorBuffer, const bool depthBuffer )
{
