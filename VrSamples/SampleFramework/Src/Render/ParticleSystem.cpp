		...
		const ovrGpuState & gpuState, bool const sortParticles )
{
...

	{
		...
		Program = OVRFW::GlProgram::Build( particleVertexSrc, particleFragmentSrc, uniformParms, uniformCount );
	}
...
void ovrParticleSystem::Frame( const OVRFW::ovrApplFrameIn & frame, const ovrTextureAtlas & atlas, const Matrix4f & centerEyeViewMatrix )
{
	...
	if ( activeCount > 0 )
	{
		...
		for ( int i = 0; i < activeCount; ++i )
		{
			...
			// This always aligns the particle to the direction of the particle to the view position.
...
ovrParticleSystem::handle_t ovrParticleSystem::AddParticle( const OVRFW::ovrApplFrameIn & frame,
...
void ovrParticleSystem::UpdateParticle( const OVRFW::ovrApplFrameIn & frame, const handle_t handle,
