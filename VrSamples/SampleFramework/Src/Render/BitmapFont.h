...
public:
	...
	virtual ovrSurfaceDef 	TextSurface( const char * text,
			float scale, const OVR::Vector4f & color, HorizontalJustification hjust,
...
public:
	...
	virtual OVR::Vector3f	DrawText3D( BitmapFont const & font, const fontParms_t & flags,
						        const OVR::Vector3f & pos, OVR::Vector3f const & normal, OVR::Vector3f const & up,
						        float const scale, OVR::Vector4f const & color, char const * text ) = 0;
	virtual OVR::Vector3f	DrawText3Df( BitmapFont const & font, const fontParms_t & flags,
						        const OVR::Vector3f & pos, OVR::Vector3f const & normal, OVR::Vector3f const & up,
						        float const scale, OVR::Vector4f const & color, char const * text, ... ) = 0;

	virtual OVR::Vector3f	DrawTextBillboarded3D( BitmapFont const & font, fontParms_t const & flags,
						        OVR::Vector3f const & pos, float const scale, OVR::Vector4f const & color, 
                                char const * text ) = 0;
	virtual OVR::Vector3f	DrawTextBillboarded3Df( BitmapFont const & font, fontParms_t const & flags,
						        OVR::Vector3f const & pos, float const scale, OVR::Vector4f const & color, 
                                char const * fmt, ... ) = 0;
