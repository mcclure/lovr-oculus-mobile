...
static GLuint CompileShader( GLenum shaderType, const char * directives, const char * src, GLint programVersion )
{
	...
	// TODO: If a c string isn't passed here, the previous contents of srcString (ie version info) are corrupted. Determine why.
							...
							bool abortOnError )
{
	return Build( NULL, vertexSrc, NULL, fragmentSrc, parms, numParms,
					requestedProgramVersion, abortOnError );
}
							...
							bool abortOnError )
{
	...
	{
		Free( p );
		ALOG( "GlProgram: CompileShader GL_VERTEX_SHADER program failed: \n```%s\n```\n\n", vertexSrc );
		if ( abortOnError )
		{
	...
	{
		Free( p );
		ALOG( "GlProgram: CompileShader GL_FRAGMENT_SHADER program failed: \n```%s\n```\n\n", fragmentSrc );
		if ( abortOnError )
		{
	...
	for ( int i = 0; i < numParms; ++i )
	{
		...
		ALOG( "   GlProgram[ %d ]: Uniforms[%d] = `%s` %s", 
			p.Program, i, parms[i].Name, parms[i].Type == ovrProgramParmType::TEXTURE_SAMPLED ? "Texture" : "" );
...
#if OVR_USE_UNIFORM_NAMES	
		p.UniformNames[i] = std::string( parms[i].Name );
		ALOGW( "   GlProgram[ %d ]: Uniforms[%d] = `%s` %s NOT BOUND / USED", 
			p.Program, i, parms[i].Name, parms[i].Type == ovrProgramParmType::TEXTURE_SAMPLED ? "Texture" : "" );
#else
