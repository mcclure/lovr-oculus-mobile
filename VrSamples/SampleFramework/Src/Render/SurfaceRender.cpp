...
static void ChangeGpuState( const ovrGpuState oldState, const ovrGpuState newState, bool force = false )
{
			...
			)
	{
		...
		{
			GL( glBlendFuncSeparate( newState.blendSrc, newState.blendDst,
										   ...
										   const int numViews )
{
	...
	//ALOG( "UpdateSceneMatrices: RequiresUpdate %d, CurrIdx %d", requiresUpdate, CurrentSceneMatricesIdx );
	...
	for ( const ovrDrawSurface & drawSurface : surfaceList )
	{
		...
		if ( cmd.Program.IsValid() )
		{
			...
			// Update globally defined system level uniforms.
			{
				...
				GL( glUniformMatrix4fv( cmd.Program.ModelMatrix.Location, 1, GL_TRUE, drawSurface.modelMatrix.M[0] ) );

				if ( cmd.Program.SceneMatrices.Location >= 0 )
				{
					GL( glBindBufferBase( GL_UNIFORM_BUFFER, cmd.Program.SceneMatrices.Binding, SceneMatrices[sceneMatricesIdx].GetBuffer() ) );
				}
			}
				...
				for ( int i = 0; i < ovrUniform::MAX_UNIFORMS && !uniformsDone; ++i )
				{
					...
					switch( cmd.Program.Uniforms[i].Type )
					{
						case ovrProgramParmType::INT:
						{
							...
							{
								GL( glUniform1iv( parmLocation, 1,
						...
						case ovrProgramParmType::INT_VECTOR2:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform2iv( parmLocation, 1,
									static_cast< const int * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						case ovrProgramParmType::INT_VECTOR3:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform3iv( parmLocation, 1,
									static_cast< const int * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						case ovrProgramParmType::INT_VECTOR4:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform4iv( parmLocation, 1,
									static_cast< const int * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						...
						case ovrProgramParmType::FLOAT_VECTOR2:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform2fv( parmLocation, 1,
									static_cast< const float * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						case ovrProgramParmType::FLOAT_VECTOR3:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform3fv( parmLocation, 1,
									static_cast< const float * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						case ovrProgramParmType::FLOAT_VECTOR4:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								GL( glUniform4fv( parmLocation, 1,
									static_cast< const float * >( cmd.UniformData[i].Data ) ) );
							}
						}
						break;
						case ovrProgramParmType::FLOAT_MATRIX4:
						{
							if ( parmLocation >= 0 && cmd.UniformData[i].Data != NULL )
							{
								if ( cmd.UniformData[i].Count > 1 )
								{
									...
									GL( glUniformMatrix4fv( parmLocation, numJoints,
											GL_FALSE,
											static_cast< const float * >( &transposedJoints[0].M[0][0] ) ) );
								}
								else
								{
									GL( glUniformMatrix4fv( parmLocation, cmd.UniformData[i].Count,
											GL_TRUE,
											static_cast< const float * >( cmd.UniformData[i].Data ) ) );
								}
							}
						...
						case ovrProgramParmType::TEXTURE_SAMPLED:
						{
							...
							if ( parmBinding >= 0 && cmd.UniformData[i].Data != NULL )
							{
								...
								if ( currentTextures[parmBinding] != texture.texture )
								{
									...
									GL( glBindTexture( texture.target ? texture.target : GL_TEXTURE_2D, texture.texture ) );
								}
							}
						}
						break;
						case ovrProgramParmType::BUFFER_UNIFORM:
						{
							...
							if ( parmBinding >= 0 && cmd.UniformData[i].Data != NULL )
							{
								...
								if ( currentBuffers[parmBinding] != buffer.GetBuffer() )
								{
									...
									GL( glBindBufferBase( GL_UNIFORM_BUFFER, parmBinding, buffer.GetBuffer() ) );
								}
							}
						}
						break;
		...
		{
			ALOG( "Drawing %s vao=%d vb=%d primitive=0x%04x indexCount=%d IndexType=0x%04x ", 
		...
		// Bind all the vertex and element arrays
		{
			...
			{
				GL( glDrawElementsInstanced( surfaceDef.geo.primitiveType, surfaceDef.geo.indexCount, surfaceDef.geo.IndexType, NULL, surfaceDef.numInstances ) );
			}
			else
			{
				GL( glDrawElements( surfaceDef.geo.primitiveType, surfaceDef.geo.indexCount, surfaceDef.geo.IndexType, NULL ) );
			}
		}
