						...
						const int mipcount, const bool useSrgbFormat, const bool imageSizeStored )
{
	...
	for ( int i = 0; i < mipcount; i++ )
	{
		...
		{
			ALOG( "%s: Mip level %d exceeds buffer size (%d > %td)", fileName, i, mipSize, ptrdiff_t( endOfBuffer - level ) );
			glBindTexture( GL_TEXTURE_2D, 0 );
			return GlTexture( texId, GL_TEXTURE_2D, width, height );
		}
						...
						const int mipcount, const bool useSrgbFormat, const bool imageSizeStored )
{
	...
	if ( !TextureFormatToGlFormat( format, useSrgbFormat, glFormat, glInternalFormat ) )
	{
		ALOG( "%s: TextureFormatToGlFormat 0x%x %s failed", fileName, (int)format, useSrgbFormat ? "true" : "false");
		return GlTexture( 0, 0, 0 );
	}
	...
	for ( int i = 0; i < mipcount; i++ )
	{
		...
		for ( int side = 0; side < 6; side++ )
		{
			if ( mipSize <= 0 || mipSize > endOfBuffer - level )
			{
				ALOG( "%s: Mip level %d exceeds buffer size (%u > %td)", fileName, i, mipSize, ptrdiff_t( endOfBuffer - level ) );
				glBindTexture( GL_TEXTURE_CUBE_MAP, 0 );
				return GlTexture( texId, GL_TEXTURE_CUBE_MAP, width, height );
			}
						...
						bool useSrgbFormat, bool noMipMaps, int & width, int & height )
{
	...
	switch ( header.PixelFormat )
	{
		...
		default:
			ALOG( "%s: Unknown PVR texture format %u, size %ix%i", fileName, static_cast< uint32_t >( header.PixelFormat ), width, height );
			return GlTexture( 0, 0, 0 );
	}
	...
	const std::uint32_t mipCount = ( noMipMaps ) ? 1 : std::max< std::uint32_t >( static_cast<std::uint32_t>( 1u ), header.MipMapCount );
...
unsigned char * LoadPVRBuffer( const char * fileName, int & width, int & height )
{
	...
	switch ( header.PixelFormat )
	{
 		case 578721384203708274llu:	format = Texture_RGBA;		break;
		default:
			ALOG( "Unknown PVR texture format %u, size %ix%i", static_cast< uint32_t >( header.PixelFormat ), width, height );
			return nullptr;
	}
						...
						bool useSrgbFormat, bool noMipMaps, int & width, int & height )
{
	...
	{
		ALOG( "%s: KTX file has unsupported number of array elements %d", fileName, header.numberOfArrayElements );
		return GlTexture( 0, 0, 0 );
	}
	...
	{
		ALOG( "%s: KTX file has unsupported glFormat %d, glInternalFormat %d", fileName, header.glFormat, header.glInternalFormat );
		return GlTexture( 0, 0, 0 );
	}
	...
	const std::uint32_t mipCount = ( noMipMaps ) ? 1 : std::max< std::uint32_t >( static_cast<std::uint32_t>( 1u ), header.numberOfMipmapLevels );
		...
		const TextureFlags_t & flags, int & width, int & height )
{
	...
	if ( fileName == nullptr || buffer == nullptr || bufferSize < 1 )
	{
		...
		ALOG( "LoadTextureFromBuffer - can't load from empties: fileName = %s buffer = %p bufferSize = %d",
