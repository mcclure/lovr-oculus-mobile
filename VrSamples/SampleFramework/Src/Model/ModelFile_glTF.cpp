...
bool ReadSurfaceDataFromAccessor( std::vector< _type_ > & out, ModelFile & modelFile, const int index, const ModelAccessorType type, const int componentType, const int count )
{
	...
	if ( index >= 0 )
	{
		...
		{
			ALOGW( "Error: Invalid index on gltfPrimitive accessor %d %d", index, static_cast< int >( modelFile.Accessors.size() ) );
			loaded = false;
		}
		...
		{
			ALOGW( "Error: Invalid count on gltfPrimitive accessor %d %d %d", index, count, accessor->count );
			loaded = false;
		}
		if ( accessor->type != type )
		{
			ALOGW( "Error: Invalid type on gltfPrimitive accessor %d %d %d", index, type, accessor->type );
			loaded = false;
		}

		if ( accessor->componentType != componentType )
		{
			ALOGW( "Error: Invalid componentType on gltfPrimitive accessor %d %d %d", index, componentType, accessor->componentType );
			loaded = false;
		}
		...
		if ( accessor->bufferView->byteStride > 0 )
		{
			if ( accessor->bufferView->byteStride != (int)sizeof( out[0] ) )
			{
				...
				{
					ALOGW( "Error: bytestride is %d, thats smaller thn %d", accessor->bufferView->byteStride, ( int )sizeof( out[0] ) );
					loaded = false;
				}
				else
				{
		...
		{
			ALOGW( "Error: accessor requesting too much data in gltfPrimitive %d %d %d", index, ( int )accessor->bufferView->byteLength, ( int )( offset + copySize ) );
			loaded = false;
		}
	...
	else
	{
		...
		if ( models.IsObject() )
		{
			...
			{ // ASSET
				...
				if ( OVR::OVR_stricmp( versionString.c_str(), "2.0" ) != 0 && OVR::OVR_stricmp( minVersion.c_str(), "2.0" ) != 0 )
				{
					ALOGW( "Error: Invalid version number '%s' on gltfFile, currently only version 2.0 supported", versionString.c_str() );
					loaded = false;
				}
			} // END ASSET
				...
				if ( accessors.IsArray() )
				{
					while ( !accessors.IsEndOfArray() && loaded)
					{
						...
						if ( accessor.IsObject() )
						{
							...
							if ( OVR::OVR_stricmp( type.c_str(), "SCALAR" ) == 0 ) { newGltfAccessor.type = ACCESSOR_SCALAR; componentCount = 1; }
							else if ( OVR::OVR_stricmp( type.c_str(), "VEC2" ) == 0 ) { newGltfAccessor.type = ACCESSOR_VEC2; componentCount = 2; }
							else if ( OVR::OVR_stricmp( type.c_str(), "VEC3" ) == 0 ) { newGltfAccessor.type = ACCESSOR_VEC3; componentCount = 3; }
							else if ( OVR::OVR_stricmp( type.c_str(), "VEC4" ) == 0 ) { newGltfAccessor.type = ACCESSOR_VEC4; componentCount = 4; }
							else if ( OVR::OVR_stricmp( type.c_str(), "MAT2" ) == 0 ) { newGltfAccessor.type = ACCESSOR_MAT2; componentCount = 4; }
							else if ( OVR::OVR_stricmp( type.c_str(), "MAT3" ) == 0 ) { newGltfAccessor.type = ACCESSOR_MAT3; componentCount = 9; }
							else if ( OVR::OVR_stricmp( type.c_str(), "MAT4" ) == 0 ) { newGltfAccessor.type = ACCESSOR_MAT4; componentCount = 16; }
							else
							{
					...
					while ( !materials.IsEndOfArray() )
					{
						...
						if ( material.IsObject() )
						{
							...
							if ( emissiveFactor != nullptr )
							{
								...
								{
									ALOGW( "Error: Invalid Itemcount on emissiveFactor for gltfMaterial" );
									loaded = false;
								}
							...
							if ( OVR::OVR_stricmp( alphaModeString.c_str(), "OPAQUE" ) == 0 ) { newGltfMaterial.alphaMode = ALPHA_MODE_OPAQUE; }
							else if ( OVR::OVR_stricmp( alphaModeString.c_str(), "MASK" ) == 0 ) { newGltfMaterial.alphaMode = ALPHA_MODE_MASK; }
							else if ( OVR::OVR_stricmp( alphaModeString.c_str(), "BLEND" ) == 0 ) { newGltfMaterial.alphaMode = ALPHA_MODE_BLEND; }
							else
							{
							...
							const OVR::JsonReader pbrMetallicRoughness = material.GetChildByName( "pbrMetallicRoughness" );
							if ( pbrMetallicRoughness.IsObject() )
							{
								...
								if ( baseColorFactor != nullptr )
								{
									...
									{
										ALOGW( "Error: Invalid Itemcount on baseColorFactor for gltfMaterial" );
										loaded = false;
									}
								...
								const OVR::JsonReader baseColorTexture = pbrMetallicRoughness.GetChildByName( "baseColorTexture" );
								if ( baseColorTexture.IsObject() )
								{
									...
									{
										ALOGW( "Error: Invalid baseColorTexture index in gltfMaterial" );
										loaded = false;
									}
								...
								const OVR::JsonReader metallicRoughnessTexture = pbrMetallicRoughness.GetChildByName( "metallicRoughnessTexture" );
								if ( metallicRoughnessTexture.IsObject() )
								{
									int index = metallicRoughnessTexture.GetChildInt32ByName( "index", -1 );
									if ( index < 0 || index >= static_cast< int >( modelFile.TextureWrappers.size() ) )
									{
										ALOGW( "Error: Invalid metallicRoughnessTexture index in gltfMaterial" );
										loaded = false;
									}
							...
							//normalTexture
							const OVR::JsonReader normalTexture = material.GetChildByName( "normalTexture" );
							...
							//occlusionTexture
							const OVR::JsonReader occlusionTexture = material.GetChildByName( "occlusionTexture" );
							...
							//emissiveTexture
							const OVR::JsonReader emissiveTexture = material.GetChildByName( "emissiveTexture" );
					...
					while ( !meshes.IsEndOfArray() && loaded )
					{
						...
						if ( mesh.IsObject() )
						{
							...
							{ // SURFACES (gltf primative)
								...
								{
									const OVR::JsonReader primitive( primitives.GetNextArrayElement() );
									...
									if ( mode != GL_TRIANGLES )
									{
										...
										ALOGW( "Error: Mode other then TRIANGLE (4) not currently supported on gltfPrimitive" );
										loaded = false;
									}

									// #TODO: implement morph targets

									const OVR::JsonReader attributes( primitive.GetChildByName( "attributes" ) );
									...
									{ // POSITION and BOUNDS
										...
										{
											ALOGW( "Error: Invalid positionAccessor on surface %s", newGltfSurface.surfaceDef.surfaceName.c_str() );
											loaded = false;
										}
										else if ( !positionAccessor->minMaxSet )
										{
											ALOGW( "Error: no min and max set on positionAccessor on surface %s", newGltfSurface.surfaceDef.surfaceName.c_str() );
											loaded = false;
										}
										else
										{
									...
									{ 
										...
										if ( jointIndex >= 0 && jointIndex < static_cast< int >( modelFile.Accessors.size() ) )
										{
											...
											{
												ALOGW( "invalid component type %d on joints_0 accessor on model %s", acc.componentType, modelFile.FileName.c_str() );
												loaded = false;
											}
									...
									{
										ALOGW( "Error: Currently, only componentType of %d supported for indices, %d requested", GL_UNSIGNED_SHORT, modelFile.Accessors[indicesIndex].componentType );
										loaded = false;
									}
									...
									if ( newGltfSurface.material->alphaMode == ALPHA_MODE_MASK )
									{
										...
										ALOGW( "gltfAlphaMode ALPHA_MODE_MASK requested, doing ALPHA_MODE_BLEND instead" );
									...
									else if ( newGltfSurface.material->alphaMode == ALPHA_MODE_BLEND || materialParms.Transparent )
									{
										...
										{
											ALOGW( "gltfAlphaMode is %d but treating at ALPHA_MODE_BLEND due to materialParms.Transparent", newGltfSurface.material->alphaMode );
										}
										newGltfSurface.surfaceDef.graphicsCommand.GpuState.blendEnable = ovrGpuState::BLEND_ENABLE;
										newGltfSurface.surfaceDef.graphicsCommand.GpuState.depthMaskEnable = false;
										newGltfSurface.surfaceDef.graphicsCommand.GpuState.blendSrc = GL_SRC_ALPHA;
									...
									{
										newGltfSurface.surfaceDef.graphicsCommand.Textures[0] = newGltfSurface.material->baseColorTextureWrapper->image->texid;
										...
										if ( newGltfSurface.material->emissiveTextureWrapper != nullptr )
										{
											...
											newGltfSurface.surfaceDef.graphicsCommand.Textures[1] = newGltfSurface.material->emissiveTextureWrapper->image->texid;

											if ( skinned )
											{
												if ( programs.ProgSkinnedBaseColorEmissivePBR == nullptr )
												{
													ALOGE_FAIL( "No ProgSkinnedBaseColorEmissivePBR set" );
												}
				...
				if ( cameras.IsArray() && loaded )
				{
					while ( !cameras.IsEndOfArray() )
					{
						...
						if ( camera.IsObject() )
						{
							...
							else if ( OVR::OVR_stricmp( cameraTypeString.c_str(), "orthographic" ) == 0 )
							{
								newGltfCamera.type = MODEL_CAMERA_TYPE_ORTHOGRAPHIC;
							}
							else { 
								ALOGW( "Error: Invalid camera type on gltfCamera %s", cameraTypeString.c_str() );
								loaded = false;
							}

							if ( newGltfCamera.type == MODEL_CAMERA_TYPE_ORTHOGRAPHIC )
							{
								const OVR::JsonReader orthographic( camera.GetChildByName( "orthographic" ) );
								if ( !orthographic.IsObject() )
								{
									ALOGW( "Error: No orthographic object on orthographic gltfCamera" );
									loaded = false;
								}
							...
							{
								const OVR::JsonReader perspective( camera.GetChildByName( "perspective" ) );
					...
					while ( !nodes.IsEndOfArray() )
					{
						...
						if ( node.IsObject() )
						{
							...
							if ( matrixReader.IsArray() )
							{
								...
								// ROTATION
								const float rcpScaleX = OVR::RcpSqrt( matrix.M[0][0] * matrix.M[0][0] + matrix.M[0][1] * matrix.M[0][1] + matrix.M[0][2] * matrix.M[0][2] );
								const float rcpScaleY = OVR::RcpSqrt( matrix.M[1][0] * matrix.M[1][0] + matrix.M[1][1] * matrix.M[1][1] + matrix.M[1][2] * matrix.M[1][2] );
								const float rcpScaleZ = OVR::RcpSqrt( matrix.M[2][0] * matrix.M[2][0] + matrix.M[2][1] * matrix.M[2][1] + matrix.M[2][2] * matrix.M[2][2] );
								const float m[9] =
								{
							...
							if ( cameraIndex >= 0 )
							{
								...
								{
									ALOGW( "Error: Invalid camera index %d on gltfNode", cameraIndex );
									loaded = false;
								}
							...
							if ( children.IsArray() )
							{
								...
								while ( !children.IsEndOfArray() )
								{
									...
									{
										ALOGW( "Error: Invalid child node index %d for %d in gltfNode", childIndex, nodeIndex );
										loaded = false;
									}
							...
							if ( samplers.IsArray() )
							{
								while ( !samplers.IsEndOfArray() && loaded )
								{
									...
									if ( sampler.IsObject() )
									{
										...
										{
											ALOGW( "bad input index %d on sample on %s", inputIndex, modelAnimation.name.c_str() );
											loaded = false;
										}
										else
										{
											...
											{
												ALOGW( "animation sampler input not of type GL_FLOAT on '%s'", modelAnimation.name.c_str() );
												loaded = false;
											}
										}
										...
										{
											ALOGW( "bad input outputIndex %d on sample on %s", outputIndex, modelAnimation.name.c_str() );
											loaded = false;
										}
										else
										{
										...
										if ( OVR::OVR_stricmp( interpolation.c_str(), "LINEAR" ) == 0 )
										{
											modelAnimationSampler.interpolation = MODEL_ANIMATION_INTERPOLATION_LINEAR;
										}
										else if ( OVR::OVR_stricmp( interpolation.c_str(), "STEP" ) == 0 )
										{
											modelAnimationSampler.interpolation = MODEL_ANIMATION_INTERPOLATION_STEP;
										}
										else if ( OVR::OVR_stricmp( interpolation.c_str(), "CATMULLROMSPLINE" ) == 0 )
										{
											modelAnimationSampler.interpolation = MODEL_ANIMATION_INTERPOLATION_CATMULLROMSPLINE;
										}
										else if ( OVR::OVR_stricmp( interpolation.c_str(), "CUBICSPLINE" ) == 0 )
										{
											modelAnimationSampler.interpolation = MODEL_ANIMATION_INTERPOLATION_CUBICSPLINE;
										}
										else
										{
											ALOGW( "Error: Invalid interpolation type '%s' on sampler on animtion '%s'", interpolation.c_str(), modelAnimation.name.c_str() );
											loaded = false;
										}
										...
										if ( loaded )
										{
											if ( modelAnimationSampler.interpolation == MODEL_ANIMATION_INTERPOLATION_LINEAR || modelAnimationSampler.interpolation == MODEL_ANIMATION_INTERPOLATION_STEP )
											{
												...
												{
													ALOGW( "input and output have different counts on sampler  on animation '%s'", modelAnimation.name.c_str() );
													loaded = false;
												}
												if ( modelAnimationSampler.input->count < 2 )
												{
													ALOGW( "invalid number of samples on animation sampler input %d '%s'", modelAnimationSampler.input->count, modelAnimation.name.c_str() );
													loaded = false;
												}
											}
											else if ( modelAnimationSampler.interpolation == MODEL_ANIMATION_INTERPOLATION_CATMULLROMSPLINE )
											{
												...
												{
													ALOGW( "input and output have invalid counts on sampler on animation '%s'", modelAnimation.name.c_str() );
													loaded = false;
												}
											...
											{
												ALOGW( "unkown animaiton interpolation on '%s'", modelAnimation.name.c_str() );
												loaded = false;
											}
										}
							...
							if ( channels.IsArray() )
							{
								while ( !channels.IsEndOfArray() && loaded )
								{
									...
									if ( channel.IsObject() )
									{
										...
										{
											ALOGW( "bad samplerIndex %d on channel on %s", samplerIndex, modelAnimation.name.c_str() );
											loaded = false;
										}
										else
										{
										...
										const OVR::JsonReader target = channel.GetChildByName( "target" );
										if ( target.IsObject() )
										{
											...
											{
												ALOGW( "bad nodeIndex %d on target on '%s'", nodeIndex, modelAnimation.name.c_str() );
												loaded = false;
											}
											else
											{
											...
											if ( OVR::OVR_stricmp( path.c_str(), "translation" ) == 0 )
											{
												modelAnimationChannel.path = MODEL_ANIMATION_PATH_TRANSLATION;
											}
											else if ( OVR::OVR_stricmp( path.c_str(), "rotation" ) == 0 )
											{
												modelAnimationChannel.path = MODEL_ANIMATION_PATH_ROTATION;
											}
											else if ( OVR::OVR_stricmp( path.c_str(), "scale" ) == 0 )
											{
												modelAnimationChannel.path = MODEL_ANIMATION_PATH_SCALE;
											}
											else if ( OVR::OVR_stricmp( path.c_str(), "weights" ) == 0 )
											{
												modelAnimationChannel.path = MODEL_ANIMATION_PATH_WEIGHTS;
											}
											else
											{
												ALOGW( " bad path '%s' on target on '%s'", path.c_str(), modelAnimation.name.c_str() );
												loaded = false;
											}
										}
										else
										{
											ALOGW( "bad target object on '%s'", modelAnimation.name.c_str() );
											loaded = false;
										}
				...
				if ( skins.IsArray() )
				{
					while ( !skins.IsEndOfArray() && loaded )
					{
						...
						if ( skin.IsObject() )
						{
							...
							{
								ALOGW( "inverseBindMatrices %d higher then number of accessors on model: %s", bindMatricesAccessorIndex, modelFile.FileName.c_str() );
								loaded = false;
							}
							...
							if ( joints.IsArray() )
							{
								while ( !joints.IsEndOfArray() && loaded )
								{
									...
									{
										ALOGW( "bad jointindex %d on skin on model: %s", jointIndex, modelFile.FileName.c_str() );
										loaded = false;
									}
									else
									{
										ALOGW( " SKIN - jointIndex: %02d name: %s", jointIndex, modelFile.Nodes[jointIndex].name.c_str() );
									}
							...
							{
								ALOGW( "%d joints on skin on model: %s, currently only %d allowed ", static_cast< int >( newSkin.jointIndexes.size() ), modelFile.FileName.c_str(), MAX_JOINTS );
								loaded = false;
							}
			...
			{ // verify skin indexes on nodes
				for ( int i = 0; i < static_cast< int >( modelFile.Nodes.size() ); i++ )
				{
					...
					{
						ALOGW( "bad skin index %d on node %d on model: %s", modelFile.Nodes[i].skinIndex, i, modelFile.FileName.c_str() );
						loaded = false;
					}
				}
			}
	...
	ModelGeo * outModelGeo )
{
	...
	int gltfJsonLength = 0;
	{
		...
		for ( int ret = unzGoToFirstFile( zfp ); ret == UNZ_OK; ret = unzGoToNextFile( zfp ) )
		{
			...
			if ( OVR::OVR_stricmp( extension, ".gltf" ) == 0 )
			{
				...
				if ( buffer == nullptr )
				{
					ALOGW( "LoadModelFile_glTF_OvrScene:Failed to read %s from %s", entryName, fileName );
					continue;
				}
	...
	if ( json == nullptr )
	{
		ALOGW( "LoadModelFile_glTF_OvrScene: Error loading %s : %s", modelFilePtr->FileName.c_str(), error );
		loaded = false;
	}
	else
	{
				...
				if ( buffers.IsArray() )
				{
					while ( !buffers.IsEndOfArray() && loaded )
					{
						...
						if ( bufferReader.IsObject() )
						{
							...
							{
								ALOGW( "%d byteLength > bufferLength loading gltfBuffer %d", (int)newGltfBuffer.byteLength, bufferLength );
								loaded = false;
							}
					...
					while ( !images.IsEndOfArray() )
					{
						...
						if ( image.IsObject() )
						{
							...
							if ( bufferView >= 0 )
							{
								...
								ALOGW( "Loading images from bufferView currently unsupported, defaulting image" );
							...
							else
							{
								...
								if ( OVR::OVR_stricmp( uri.c_str() + (uri.length() - 4), ".ktx" ) != 0 )
								{
									// #TODO: Try looking for a ktx image before we load the non ktx image.
									ALOGW( "Loading images other then ktx is not advised. %s", uri.c_str() );
	...
	ModelGeo * outModelGeo )
{
	...
	if ( loaded && fileDataRemainingLength > sizeof( uint32_t ) * 2 )
	{
		...
		if ( loaded )
		{
			...
			if ( json == nullptr )
			{
				ALOGW( "LoadModelFile_glB: Error Parsing JSON %s : %s", modelFilePtr->FileName.c_str(), error );
				loaded = false;
			}
		}
					...
					if ( buffers.IsArray() )
					{
						while ( !buffers.IsEndOfArray() && loaded )
						{
							...
							if ( bufferReader.IsObject() && loaded )
							{
								...
								if ( !uri.empty() )
								{
									ALOGW( "Loading buffers with an uri currently unsupported in glb" );
									loaded = false;
								}
					...
					if ( images.IsArray() )
					{
						while ( !images.IsEndOfArray() )
						{
							...
							if ( image.IsObject() )
							{
								...
								{
									ALOGW( "Loading images from othen then bufferView currently unsupported in glBfd, defaulting image" );
									// Create a default texture.
									LoadModelFileTexture( modelFile, "DefaultImage", nullptr, 0, materialParms );
								}
							}
