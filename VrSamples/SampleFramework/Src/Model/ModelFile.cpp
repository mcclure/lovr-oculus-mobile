							...
							const char * buffer, const int size, const MaterialParms & materialParms )
{
	...
	// ALOG( ( tex.texid.target == GL_TEXTURE_CUBE_MAP ) ? "GL_TEXTURE_CUBE_MAP: %s" : "GL_TEXTURE_2D: %s", textureName );
	...
	ModelGeo * outModelGeo = nullptr )
{
	...
	if ( modelFilePtr )
	{
		...
		for ( int i = 0; i < static_cast< int >( modelFilePtr->Models.size() ); i++ )
		{
			...
			{
				ovrGraphicsCommand & gc = * const_cast< ovrGraphicsCommand * >( &m.surfaces[j].surfaceDef.graphicsCommand );
