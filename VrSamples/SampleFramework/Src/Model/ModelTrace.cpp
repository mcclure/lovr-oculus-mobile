...
bool ModelTrace::Validate( const bool fullVerify ) const
{
	...
	if ( fullVerify )
	{
		...
		for ( int i = 0; i < static_cast< int >( nodes.size() ); ++i )
		{
			...
			{
				ALOG( "ModelTrace::Verify - leftChildIndex of %i for node %i is out of range, max %i", leftChildIndex, i, static_cast< int >( nodes.size() ) - 1 );
				return false;
			}
			if ( rightChildIndex < 0 || rightChildIndex >= static_cast< int >( nodes.size() ) )
			{
				ALOG( "ModelTrace::Verify - rightChildIndex of %i for node %i is out of range, max %i", leftChildIndex, i, static_cast< int >( nodes.size() ) - 1 );
				return false;
			}
		}
		...
		for ( int i = 0; i < static_cast< int >( leafs.size() ); ++i )
		{
			...
			for ( int j = 0; j < RT_KDTREE_MAX_LEAF_TRIANGLES; ++j )
			{
				...
				if ( triIndex < 0 )
				{
					...
					{
						ALOG( "ModelTrace::Verify - Leaf %i has an out of range overflow index %i at index %i, max %i", i, overflowIndex, j, numTris - 1 );
						return false;
					}
				...
				{
					ALOG( "ModelTrace::Verify - Leaf %i has an out of range triangle of index %i at index %i, max %i", i, triIndex, j, numTris - 1 );
					return false;
				}
			}
		}
		...
		for ( int i = 0; i < static_cast< int >( overflow.size() ); ++i )
		{
			...
			{
				ALOG( "ModelTrace::Verify - overflow index %i value %i is out of range, max %i", i, overflow[i], numTris - 1 );
				return false;
			}
		}
		...
		for ( int i = 0; i < static_cast< int >( indices.size() ); ++i )
		{
			...
			{
				ALOG( "Index %i value %i is out of range, max %i", i, indices[i], static_cast< int >( vertices.size() ) - 1 );
				return false;
			}
		}
	}
