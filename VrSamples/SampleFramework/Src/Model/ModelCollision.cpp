...
bool CollisionPolytope::TestRay( const OVR::Vector3f & start, const OVR::Vector3f & dir, float & length, OVR::Planef * plane ) const
{
...
bool ModelCollision::TestRay( const OVR::Vector3f & start, const OVR::Vector3f & dir, float & length, OVR::Planef * plane ) const
{
	bool clipped = false;
	for ( int i = 0; i < static_cast< int >( Polytopes.size() ); i++ )
	{
	    ...
	    )
{
...

	{
		...
		else
		{
			...
			const OVR::Vector3f slideDirection = ( moveDirection - fowardCollisionPlane.N * intoPlane );
