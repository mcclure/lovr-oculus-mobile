	...
	ModelGeo * outModelGeo )
{
	...
	if ( json == nullptr )
	{
		ALOGW( "LoadModelFile_OvrScene_Json: Error loading %s : %s", modelFile.FileName.c_str(), error );
		return false;
	}
	...
	{
		ALOGW( "LoadModelFile_OvrScene_Json: model already has data, replacing with %s", modelFile.FileName.c_str() );
	...
	if ( models.IsObject() )
	{
		...
		if ( render_model.IsObject() )
		{
			...
			if ( texture_array.IsArray() )
			{
				while ( !texture_array.IsEndOfArray() )
				{
					...
					if ( texture.IsObject() )
					{
						...
						{
							if ( OVR::OVR_stricmp( modelFile.Textures[i].name.c_str(), name.c_str() ) == 0 )
							{
								break;
							}
						}
			...
			if ( joint_array.IsArray() )
			{
				while ( !joint_array.IsEndOfArray() )
				{
					...
					if ( joint.IsObject() )
					{
						const size_t index = static_cast< size_t >( modelFile.Nodes[nodeIndex].JointsOvrScene.size() );
						...
						OVR::StringUtils::StringTo( modelFile.Nodes[nodeIndex].JointsOvrScene[index].transform, joint.GetChildStringByName( "transform" ).c_str() );
			...
			if ( tag_array.IsArray() )
			{
				...
				while ( !tag_array.IsEndOfArray() )
				{
					...
					if ( tag.IsObject() )
					{
						...
						OVR::StringUtils::StringTo( modelFile.Tags[index].matrix, tag.GetChildStringByName( "matrix" ).c_str() );
						OVR::StringUtils::StringTo( modelFile.Tags[index].jointIndices, tag.GetChildStringByName( "jointIndices" ).c_str() );
						OVR::StringUtils::StringTo( modelFile.Tags[index].jointWeights, tag.GetChildStringByName( "jointWeights" ).c_str() );
					}
				}
			}
			...
			if ( surface_array.IsArray() )
			{
				while ( !surface_array.IsEndOfArray() )
				{
					...
					if ( surface.IsObject() )
					{
						...
						OVR::StringUtils::StringTo( modelSurface.surfaceDef.geo.localBounds, surface.GetChildStringByName( "bounds" ).c_str() );
						...
						{
							modelSurface.surfaceDef.graphicsCommand.Textures[0] = glTextures[diffuseTextureIndex];

							if ( emissiveTextureIndex >= 0 && emissiveTextureIndex < static_cast< int >( glTextures.size() ) )
							{
								modelSurface.surfaceDef.graphicsCommand.Textures[1] = glTextures[emissiveTextureIndex];
									...
									reflectionTextureIndex >= 0 && reflectionTextureIndex < static_cast< int >( glTextures.size() ) )
								{
									...
									modelSurface.surfaceDef.graphicsCommand.Textures[2] = glTextures[normalTextureIndex];
									modelSurface.surfaceDef.graphicsCommand.Textures[3] = glTextures[specularTextureIndex];
									modelSurface.surfaceDef.graphicsCommand.Textures[4] = glTextures[reflectionTextureIndex];
		...
		if ( collision_model.IsArray() )
		{
			...
			while ( !collision_model.IsEndOfArray() )
			{
				...
				if ( polytope.IsObject() )
				{
					...
					OVR::StringUtils::StringTo( modelFile.Collisions.Polytopes[index].Planes, polytope.GetChildStringByName( "planes" ).c_str() );
				}
			}
		}
		...
		const OVR::JsonReader ground_collision_model( models.GetChildByName( "ground_collision_model" ) );
		if ( ground_collision_model.IsArray() )
		{
			...
			{
				const size_t index = static_cast< size_t >( modelFile.GroundCollisions.Polytopes.size() );
				...
				if ( polytope.IsObject() )
				{
					...
					OVR::StringUtils::StringTo( modelFile.GroundCollisions.Polytopes[index].Planes, polytope.GetChildStringByName( "planes" ).c_str() );
				}
			}
		}
		...
		if ( raytrace_model.IsObject() )
		{
			...
			OVR::StringUtils::StringTo( traceModel.header.bounds, raytrace_model.GetChildStringByName( "bounds" ).c_str() );
			...
			if ( !bin.ReadArray( traceModel.nodes, traceModel.header.numNodes ) )
			{
				...
				if ( nodes_array.IsArray() )
				{
					while ( !nodes_array.IsEndOfArray() )
					{
						...
						if ( node.IsObject() )
						{
							traceModel.nodes[index].data = ( std::uint32_t )node.GetChildInt64ByName( "data" );
			...
			if ( !bin.ReadArray( traceModel.leafs, traceModel.header.numLeafs ) )
			{
				...
				if ( leafs_array.IsArray() )
				{
					while ( !leafs_array.IsEndOfArray() )
					{
						...
						if ( leaf.IsObject() )
						{
							OVR::StringUtils::StringTo( traceModel.leafs[index].triangles, RT_KDTREE_MAX_LEAF_TRIANGLES, leaf.GetChildStringByName( "triangles" ).c_str() );
							OVR::StringUtils::StringTo( traceModel.leafs[index].ropes, 6, leaf.GetChildStringByName( "ropes" ).c_str() );
							OVR::StringUtils::StringTo( traceModel.leafs[index].bounds, leaf.GetChildStringByName( "bounds" ).c_str() );
						}
					}
				}
			}
	...
	ModelGeo * outModelGeo )
{
	...
	for ( int ret = unzGoToFirstFile( zfp ); ret == UNZ_OK; ret = unzGoToNextFile( zfp ) )
	{
		...
		else if ( OVR::OVR_stricmp( extension, ".pvr" ) == 0 ||
			OVR::OVR_stricmp( extension, ".ktx" ) == 0 )
		{
