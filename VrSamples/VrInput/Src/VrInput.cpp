...
bool ovrVrInput::AppInit( const OVRFW::ovrAppContext * context )
{
	...
	ProgSingleTexture = OVRFW::GlProgram::Build( PrelitVertexShaderSrc, PrelitFragmentShaderSrc,
		PreLitUniformParms, sizeof( PreLitUniformParms ) / sizeof( ovrProgramParm ) );
...

	{
		...
		ControllerModelGear = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );

		if ( ControllerModelGear == NULL || static_cast< int >( ControllerModelGear->Models.size() ) < 1 )
		{
			ALOGE_FAIL( "Couldn't load Gear VR controller model" );
		}

		auto & gc = ControllerModelGear->Models[0].surfaces[0].surfaceDef.graphicsCommand;
		gc.UniformData[0].Data = &ControllerModelGear->Models[0].surfaces[0].surfaceDef.graphicsCommand.Textures[0];
		...
		gc.UniformData[4].Data = &ControllerModelGear->Models[0].surfaces[0].surfaceDef.graphicsCommand.Textures[1];
...

	{
		...
		MaterialParms materials;
		ControllerModelGearPreLit = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );

		if ( ControllerModelGearPreLit == NULL || static_cast< int >( ControllerModelGearPreLit->Models.size() ) < 1 )
		{
			ALOGE_FAIL( "Couldn't load prelit Gear VR controller model" );
		}

		auto & gc = ControllerModelGearPreLit->Models[0].surfaces[0].surfaceDef.graphicsCommand;
		gc.GpuState.depthEnable = gc.GpuState.depthMaskEnable = false;
		gc.GpuState.blendEnable = ovrGpuState::BLEND_ENABLE_SEPARATE;
		gc.GpuState.blendSrc = GL_SRC_ALPHA;
		gc.GpuState.blendDst = GL_ONE_MINUS_SRC_ALPHA;
	}

	{
		...
		programs.ProgLightMapped = &ProgLitSpecularWithHighlight;
		MaterialParms	materials;
		ControllerModelOculusGo = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );
...

	{
		...
		programs.ProgBaseColorPBR = &ProgSingleTexture;
		MaterialParms	materials;
		ControllerModelOculusGoPreLit = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );
...

	{
		...
		MaterialParms materials;
		ControllerModelOculusTouchLeft = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );
	...
	}
	{
		...
		programs.ProgSkinnedBaseColorEmissivePBR = &ProgOculusTouch;
		MaterialParms materials;
		ControllerModelOculusTouchRight = LoadModelFile( GuiSys->GetFileSys(), controllerModelFile, programs, materials );

		if ( ControllerModelOculusTouchRight == NULL || static_cast< int >( ControllerModelOculusTouchRight->Models.size() ) < 1 )
		{
			ALOGE_FAIL( "Couldn't load Oculus Touch for Oculus Quest Controller Controller right model" );
		}
...
static void RenderBones( const ovrApplFrameIn & frame, ovrParticleSystem * ps, ovrTextureAtlas & particleAtlas,
...
void ovrVrInput::RenderRunningFrame( const OVRFW::ovrApplFrameIn & in, OVRFW::ovrRendererOutput & out )
{
	...
	for ( int i = (int)InputDevices.size() - 1; i >= 0; --i )
	{
		...
		if ( device->GetType() == ovrControllerType_Headset && showHeadset )
		{
			...
			ovrResult result = vrapi_GetInputTrackingState( GetSessionObject(), deviceID, in.PredictedDisplayTime, &hmtTracking );

			if ( result != ovrSuccess )
			{
				ALOG( "HMTPose - Error %i getting HMT tracking!", result );
			...
			else
			{
				...
				//ALOG( "HMTPose", "Pose.r = ( %.2f, %.2f, %.2f, %.2f ), ypr( %.2f, %.2f, %.2f )",
				//		r.x, r.y, r.z, r.w,
				...
				result = vrapi_GetCurrentInputState( GetSessionObject(), deviceID, &headsetInputState.Header );
				if ( result != ovrSuccess )
				{
					ALOG( "HeadsetState", "ERROR %i getting HMT input state!", result );
					OnDeviceDisconnected( deviceID );
				}
				else
				{
		...
		/*
					ALOG( "Buttons", "%s, trackpad = %s",
							buttons.c_str(), headsetInputState.TrackpadStatus ? "down" : "up" );
					ALOG( "Trackpad", "headset ( %.2f, %.2f ), mm( %.2f, %.2f ), minmax( %.2f, %.2f, %.2f, %.2f )",
							headsetInputState.TrackpadPosition.x, headsetInputState.TrackpadPosition.y,
		...
		*/
		...
		else if ( device->GetType() == ovrControllerType_TrackedRemote )
		{
			...
			if ( deviceID != ovrDeviceIdType_Invalid )
			{
				...
				ovrResult result = vrapi_GetInputTrackingState( GetSessionObject(), deviceID, in.PredictedDisplayTime, &remoteTracking );
				if ( result != ovrSuccess )
				{
					OnDeviceDisconnected( deviceID );
					continue;
				...
				r.GetEulerAngles< Axis_Y, Axis_X, Axis_Z >( &yaw, &pitch, &roll );
				//ALOG( "MLBUPose", "Pose.r = ( %.2f, %.2f, %.2f, %.2f ), ypr( %.2f, %.2f, %.2f ), t( %.2f, %.2f, %.2f )",
				//	r.x, r.y, r.z, r.w,
				//	MATH_FLOAT_RADTODEGREEFACTOR * yaw, MATH_FLOAT_RADTODEGREEFACTOR * pitch, MATH_FLOAT_RADTODEGREEFACTOR * roll,
		...
		else if ( device->GetType() == ovrControllerType_Gamepad )
		{
			if ( deviceID != ovrDeviceIdType_Invalid )
			{
				...
				ovrResult result = vrapi_GetCurrentInputState( GetSessionObject(), deviceID, &gamepadInputState.Header );
	...
	ovrArmModel::ovrHandedness dominantHand = vrapi_GetSystemPropertyInt( java, VRAPI_SYS_PROP_DOMINANT_HAND ) == VRAPI_HAND_LEFT ? ovrArmModel::HAND_LEFT : ovrArmModel::HAND_RIGHT;
	...
	for ( int i = (int)InputDevices.size() - 1; i >= 0; --i )
	{
		...
		if ( device->GetType() == ovrControllerType_TrackedRemote )
		{
			...
			if ( ( trDevice.GetTrackedRemoteCaps().ControllerCapabilities & ovrControllerCaps_HasPositionTracking ) == 0 )
			{
				...
				RenderBones( in, ParticleSystem, *SpriteAtlas, 0, RemoteBeamRenderer, *BeamAtlas, 0,
			...
			float controllerPitch = OVR::DegreeToRad( 15.0f );
			...
			{
				Ribbons[trDevice.GetHand()]->Update( out.FrameMatrices.CenterView,
	...
	if ( hasActiveController )
	{
		if ( !LaserPointerBeamHandle.IsValid() )
		{
			LaserPointerBeamHandle = RemoteBeamRenderer->AddBeam( in, *BeamAtlas, 0, 0.032f, pointerStart, pointerEnd,
		...
		{
			RemoteBeamRenderer->UpdateBeam( in, LaserPointerBeamHandle, *BeamAtlas, 0, 0.032f, pointerStart, pointerEnd, LASER_COLOR );
		}
		...
		if ( !LaserPointerParticleHandle.IsValid() )
		{
			if ( LaserHit )
			{
				LaserPointerParticleHandle = ParticleSystem->AddParticle( in, pointerEnd, 0.0f, Vector3f( 0.0f ), Vector3f( 0.0f ),
		...
		else
		{
			if ( LaserHit )
			{
				ParticleSystem->UpdateParticle( in, LaserPointerParticleHandle, pointerEnd, 0.0f, Vector3f( 0.0f ), Vector3f( 0.0f ),
						LASER_COLOR, ovrEaseFunc::NONE, 0.0f, 0.1f, 0.1f, 0 );
			}
			else
			{
...
//		ParticleSystem->AddParticle( in, pointerStart_NoWrist, 0.0f, Vector3f( 0.0f ), Vector3f( 0.0f ),
//				Vector4f( 0.0f, 1.0f, 1.0f, 1.0f ), ovrEaseFunc::ALPHA_IN_OUT_LINEAR, 0.0f, 0.025f, 0.5f, 0 );
		ParticleSystem->AddParticle( in, pointerStart, 0.0f, Vector3f( 0.0f ), Vector3f( 0.0f ),
...
void ovrVrInput::AppRenderEye( const OVRFW::ovrApplFrameIn & in, OVRFW::ovrRendererOutput & out, int eye )
{
...
ovrResult ovrVrInput::PopulateRemoteControllerInfo( ovrInputDevice_TrackedRemote & trDevice, bool recenteredController )
{
	...
	std::string buttons;
	char temp[128];
	OVR::OVR_sprintf( temp, sizeof( temp ), "( %.2f, %.2f ) ",
...
void ovrVrInput::OnDeviceConnected( const ovrInputCapabilityHeader & capsHeader )
{
	...
	switch( capsHeader.Type )
	{
		case ovrControllerType_Headset:
			{
				ALOG( "MLBUConnect - Headset connected, ID = %u", capsHeader.DeviceID );
				ovrInputHeadsetCapabilities headsetCapabilities;
				headsetCapabilities.Header = capsHeader;
				result = vrapi_GetInputDeviceCapabilities( GetSessionObject(), &headsetCapabilities.Header );
				if ( result == ovrSuccess )
				{
					device = ovrInputDevice_Headset::Create( *this, *GuiSys, *Menu, headsetCapabilities );
				}
			}
			break;
		case ovrControllerType_TrackedRemote:
			{
				...
				result = vrapi_GetInputDeviceCapabilities( GetSessionObject(), &remoteCapabilities.Header );
				if ( result == ovrSuccess )
				{
					device = ovrInputDevice_TrackedRemote::Create( *this, *GuiSys, *Menu, remoteCapabilities );

					// populate model surfaces.
					ovrInputDevice_TrackedRemote & trDevice = *static_cast< ovrInputDevice_TrackedRemote*>( device );
					std::vector< ovrDrawSurface > & 	controllerSurfaces = trDevice.GetControllerSurfaces();
					ModelFile * modelFile = ControllerModelGear;
					if ( trDevice.GetTrackedRemoteCaps().ControllerCapabilities & ovrControllerCaps_ModelOculusGo )
					{
		...
		case ovrControllerType_Gamepad:
			{
				...
				result = vrapi_GetInputDeviceCapabilities( GetSessionObject(), &gamepadCapabilities.Header );
				if ( result == ovrSuccess )
				{
...
ovrInputDeviceBase * ovrInputDevice_Gamepad::Create( OVRFW::ovrAppl & app, OvrGuiSys & guiSys, VRMenu & menu,
...
ovrInputDeviceBase * ovrInputDevice_Headset::Create( OVRFW::ovrAppl & app, OvrGuiSys & guiSys, VRMenu & menu,
		const ovrInputHeadsetCapabilities & headsetCapabilities )
{
	...
	ALOG( "MLBUConnect - Headset caps: Button %x, Controller %x, MaxX %d, MaxY %d, SizeX %f SizeY %f",
...
ovrInputDeviceBase * ovrInputDevice_TrackedRemote::Create( OVRFW::ovrAppl & app, OvrGuiSys & guiSys, VRMenu & menu,
		const ovrInputTrackedRemoteCapabilities & remoteCapabilities )
{
	...
	if ( result == ovrSuccess )
	{
		...
		ALOG( "MLBUConnect - Controller caps: hand = %s, Button %x, Controller %x, MaxX %d, MaxY %d, SizeX %f SizeY %f",
