...
public:
	...
	const std::vector< OVR::Posef > &	GetLocalSpacePoses() const { return LocalSpacePoses; }
	const std::vector< OVR::Posef > &	GetWorldSpacePoses() const { UpdateWorldFromLocal(); return WorldSpacePoses; }
...
private:
	...
	/// The expectation is that calling any of the Transform methods will update LocalSpacePoses instantly
	/// and invalidate WorldSpaceDirty so that they can be lazily updated on demand.
