...
typedef std::vector< std::pair< ovrParticleSystem::handle_t, ovrBeamRenderer::handle_t > > jointHandles_t;
...
public:
	...
	const menuHandle_t &				GetLastHitHandle() const { return HitHandle; }
	void								SetLastHitHandle( const menuHandle_t & lastHitHandle ) { HitHandle = lastHitHandle; }

	ovrHandModel & 						GetHandModel() { return HandModel; }
	jointHandles_t & 					GetFingerJointHandles() { return FingerJointHandles; }
	ovrBeamRenderer::handle_t &			GetLaserPointerBeamHandle() { return LaserPointerBeamHandle; }
	ovrParticleSystem::handle_t &		GetLaserPointerParticleHandle() { return LaserPointerParticleHandle; }

	bool								GetIsActiveInputDevice() const { return IsActiveInputDevice; }
	void								SetIsActiveInputDevice( bool isActive ) { IsActiveInputDevice = isActive; }

	ovrHandedness						GetHand() const { return Hand; }
	inline bool 						IsLeftHand() const { return Hand == VRAPI_HAND_LEFT; }
	OVR::Posef 							GetHandPose() const { return HandPose; }
	OVR::Posef 							GetPointerPose() const { return PointerPose; }
	bool								IsPinching() const { return ((InputStateHand.InputStateStatus & ovrInputStateHandStatus_IndexPinching ) != 0); }
	bool								IsPointerValid() const { return ((InputStateHand.InputStateStatus & ovrInputStateHandStatus_PointerValid ) != 0); }
	bool								IsInSystemGesture() const { return ((InputStateHand.InputStateStatus & ovrInputStateHandStatus_SystemGestureProcessing ) != 0); }
	bool								WasPinching() const { return PreviousFramePinch; }
	bool								Clicked() const { return WasPinching() && !IsPinching(); }
...
public:
	...
	static ovrInputDeviceTrackedHand * 			Create( OVRFW::ovrAppl & app,
	...
	virtual const char *						GetName() const override { return "TrackedHand"; }
	const ovrInputHandCapabilities &			GetHandCaps() const { return Caps; }
	...
	ovrConfidence								GetHandPoseConfidence() const { return RealHandPose.HandConfidence; }
	OVR::Vector3f 								GetRayOrigin() const { return PointerPose.Transform( OVR::Vector3f( 0.0f, 0.0f, 0.0f ) ); }
	OVR::Vector3f 								GetRayEnd() const {	return PointerPose.Transform( OVR::Vector3f( 0.0f, 0.0f, -1.5f ) );	}

	const std::vector< ovrBoneCapsule > & 		GetBoneCapsules() const { return BoneCapsules; };
