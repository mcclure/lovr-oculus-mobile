...
bool ovrVkDevice_SelectPhysicalDevice( ovrVkDevice * device, ovrVkInstance * instance, const char * requiredExtensionNames, uint32_t requiredExtensionNamesSize )
{
	...
	ParseExtensionString( extensionNames, &device->enabledExtensionCount, device->enabledExtensionNames, 32 );
#if defined( _DEBUG )
	...
	for ( uint32_t physicalDeviceIndex = 0; physicalDeviceIndex < physicalDeviceCount; physicalDeviceIndex++ )
	{
		...
		for ( uint32_t queueFamilyIndex = 0; queueFamilyIndex < queueFamilyCount; queueFamilyIndex++ )
		{
			...
			ALOGV( "%-21s%c %d =%s%s (%d queues, %d priorities)\n",
...
static uint32_t VkGetMemoryTypeIndex( ovrVkDevice * device, const uint32_t typeBits,
...
void ovrGpuDevice_CreateShader( ovrVkDevice * device, VkShaderModule * shaderModule,
									...
									const int width, const int height, const int numLayers )
{
	...
	memoryAllocateInfo.memoryTypeIndex = VkGetMemoryTypeIndex( context->device, memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT );
...
{
	return	( ( type == OVR_BUFFER_TYPE_VERTEX ) ?	VK_BUFFER_USAGE_VERTEX_BUFFER_BIT :
			( ( type == OVR_BUFFER_TYPE_INDEX ) ?	VK_BUFFER_USAGE_INDEX_BUFFER_BIT :
...
{
	return	( ( type == OVR_BUFFER_TYPE_INDEX ) ?	VK_ACCESS_INDEX_READ_BIT :
			( ( type == OVR_BUFFER_TYPE_VERTEX ) ?	VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT :
							...
							const size_t dataSize, const void * data, const bool hostVisible )
{
	...
	memoryAllocateInfo.pNext = NULL;
	memoryAllocateInfo.allocationSize = memoryRequirements.size;
	memoryAllocateInfo.memoryTypeIndex = VkGetMemoryTypeIndex( context->device, memoryRequirements.memoryTypeBits, buffer->flags );
	...
	if ( data != NULL )
	{
		...
		else
		{
			...
			stagingMemoryAllocateInfo.memoryTypeIndex = VkGetMemoryTypeIndex( context->device, stagingMemoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT );
...
void ovrGpuBuffer_CreateReference( ovrVkContext * context, ovrVkBuffer * buffer, const ovrVkBuffer * other )
{
	buffer->next = NULL;
...
void ovrVkTexture_UpdateSampler( ovrVkContext * context, ovrVkTexture * texture )
{
	...
	const VkSamplerMipmapMode mipmapMode =	( ( texture->filter == OVR_TEXTURE_FILTER_NEAREST ) ? VK_SAMPLER_MIPMAP_MODE_NEAREST :
											( ( texture->filter == OVR_TEXTURE_FILTER_LINEAR  ) ? VK_SAMPLER_MIPMAP_MODE_NEAREST :
											( VK_SAMPLER_MIPMAP_MODE_LINEAR ) ) );
	const VkSamplerAddressMode addressMode =	( ( texture->wrapMode == OVR_TEXTURE_WRAP_MODE_CLAMP_TO_EDGE ) ? VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE :
												( ( texture->wrapMode == OVR_TEXTURE_WRAP_MODE_CLAMP_TO_BORDER ) ? VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER :
	...
	samplerCreateInfo.magFilter = ( texture->filter == OVR_TEXTURE_FILTER_NEAREST ) ? VK_FILTER_NEAREST : VK_FILTER_LINEAR;
	samplerCreateInfo.minFilter = ( texture->filter == OVR_TEXTURE_FILTER_NEAREST ) ? VK_FILTER_NEAREST : VK_FILTER_LINEAR;
...
{
	return	( ( usage == OVR_TEXTURE_USAGE_UNDEFINED ) ?		VK_IMAGE_LAYOUT_UNDEFINED :
			( ( usage == OVR_TEXTURE_USAGE_GENERAL ) ?			VK_IMAGE_LAYOUT_GENERAL :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_SRC ) ?		VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_DST ) ?		VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL :
			( ( usage == OVR_TEXTURE_USAGE_SAMPLED ) ?			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL :
			( ( usage == OVR_TEXTURE_USAGE_STORAGE ) ?			VK_IMAGE_LAYOUT_GENERAL :
			( ( usage == OVR_TEXTURE_USAGE_COLOR_ATTACHMENT ) ?	VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL :
			( ( usage == OVR_TEXTURE_USAGE_PRESENTATION ) ?		VK_IMAGE_LAYOUT_PRESENT_SRC_KHR : (VkImageLayout) 0 ) ) ) ) ) ) ) );
}

static VkAccessFlags AccessForTextureUsage( const ovrVkTextureUsage usage )
{
	return	( ( usage == OVR_TEXTURE_USAGE_UNDEFINED ) ?		( 0 ) :
			( ( usage == OVR_TEXTURE_USAGE_GENERAL ) ?			( 0 ) :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_SRC ) ?		( VK_ACCESS_TRANSFER_READ_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_DST ) ?		( VK_ACCESS_TRANSFER_WRITE_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_SAMPLED ) ?			( VK_ACCESS_SHADER_READ_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_STORAGE ) ?			( VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_COLOR_ATTACHMENT ) ?	( VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_PRESENTATION ) ?		( VK_ACCESS_MEMORY_READ_BIT ) : 0 ) ) ) ) ) ) ) );
}

static VkPipelineStageFlags PipelineStagesForTextureUsage( const ovrVkTextureUsage usage, const bool from )
{
	return	( ( usage == OVR_TEXTURE_USAGE_UNDEFINED ) ?		( VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_GENERAL ) ?			( VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_SRC ) ?		( VK_PIPELINE_STAGE_TRANSFER_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_TRANSFER_DST ) ?		( VK_PIPELINE_STAGE_TRANSFER_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_SAMPLED ) ?			( VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_STORAGE ) ?			( VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_COLOR_ATTACHMENT ) ?	( VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT ) :
			( ( usage == OVR_TEXTURE_USAGE_PRESENTATION ) ?		( from ? VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT : VK_PIPELINE_STAGE_ALL_COMMANDS_BIT ) : 0 ) ) ) ) ) ) ) );
}
...
bool ovrGpuTexture_CreateInternal( ovrVkContext * context, ovrVkTexture * texture, const char * fileName,
										const VkFormat format, const ovrSampleCount sampleCount,
										...
										const ovrVkTextureUsageFlags usageFlags )
{
	...
	texture->filter = ( numStorageLevels > 1 ) ? OVR_TEXTURE_FILTER_BILINEAR : OVR_TEXTURE_FILTER_LINEAR;
...
void ovrVkTriangleIndexArray_Alloc( ovrVkTriangleIndexArray * indices, const int indexCount, const ovrTriangleIndex * data )
{
...
size_t ovrGpuVertexAttributeArrays_GetDataSize( const ovrVkVertexAttribute * layout, const int vertexCount, const int attribsFlags )
{
	size_t totalSize = 0;
...
{
	const size_t dataSize = ovrGpuVertexAttributeArrays_GetDataSize( layout, vertexCount, attribsFlags );
								...
								const ovrVkTriangleIndexArray * indices )
{
	...
	{
		ovrVkBuffer_Create( context, &geometry->vertexBuffer, OVR_BUFFER_TYPE_VERTEX, attribs->dataSize, attribs->data, false );
	}
	...
	{
		ovrVkBuffer_Create( context, &geometry->indexBuffer, OVR_BUFFER_TYPE_INDEX, indices->indexCount * sizeof( indices->indexArray[0] ), indices->indexArray, false );
	}
}
...
void ovrVkGeometry_AddInstanceAttributes( ovrVkContext * context, ovrVkGeometry * geometry, const int numInstances, const int instanceAttribsFlags )
{
	...
	const size_t dataSize = ovrGpuVertexAttributeArrays_GetDataSize( geometry->layout, numInstances, geometry->instanceAttribsFlags );

	ovrVkBuffer_Create( context, &geometry->instanceBuffer, OVR_BUFFER_TYPE_VERTEX, dataSize, NULL, false );
}
...
static VkFormat ovrGpuColorBuffer_InternalSurfaceColorFormat( const ovrSurfaceColorFormat colorFormat )
{
	return	( ( colorFormat == OVR_SURFACE_COLOR_FORMAT_R8G8B8A8 ) ? VK_FORMAT_R8G8B8A8_UNORM :
			( ( colorFormat == OVR_SURFACE_COLOR_FORMAT_B8G8R8A8 ) ? VK_FORMAT_B8G8R8A8_UNORM :
			( ( colorFormat == OVR_SURFACE_COLOR_FORMAT_R5G6B5 ) ? VK_FORMAT_R5G6B5_UNORM_PACK16 :
			( ( colorFormat == OVR_SURFACE_COLOR_FORMAT_B5G6R5 ) ? VK_FORMAT_B5G6R5_UNORM_PACK16 :
			( ( VK_FORMAT_UNDEFINED ) ) ) ) ) );
}
									...
									const ovrSampleCount sampleCount, const ovrVkRenderPassType type, const int flags, const ovrVector4f * clearColor, bool isMultiview )
{
	...
	if ( sampleCount > OVR_SAMPLE_COUNT_1 )
	{
		...
		attachments[attachmentCount].loadOp = ( ( flags & OVR_RENDERPASS_FLAG_CLEAR_COLOR_BUFFER ) != 0 ) ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
	...
	if ( sampleCount <= OVR_SAMPLE_COUNT_1 || EXPLICIT_RESOLVE == 0 )
	{
		...
		attachments[attachmentCount].loadOp = ( ( flags & OVR_RENDERPASS_FLAG_CLEAR_COLOR_BUFFER ) != 0 && sampleCount <= OVR_SAMPLE_COUNT_1 ) ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
	...
	if ( renderPass->internalDepthFormat != VK_FORMAT_UNDEFINED )
	{
		...
		attachments[attachmentCount].samples = (VkSampleCountFlagBits)sampleCount;
		attachments[attachmentCount].loadOp = ( ( flags & OVR_RENDERPASS_FLAG_CLEAR_DEPTH_BUFFER ) != 0 ) ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
...
{
	return	( ( type == OVR_PROGRAM_PARM_TYPE_TEXTURE_SAMPLED ) ?	true :
			( ( type == OVR_PROGRAM_PARM_TYPE_TEXTURE_STORAGE ) ?	true :
...
{
	return	( ( type == OVR_PROGRAM_PARM_TYPE_TEXTURE_SAMPLED ) ?	VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER :
			( ( type == OVR_PROGRAM_PARM_TYPE_TEXTURE_STORAGE ) ?	VK_DESCRIPTOR_TYPE_STORAGE_IMAGE :
			( ( type == OVR_PROGRAM_PARM_TYPE_BUFFER_UNIFORM ) ?	VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER : VK_DESCRIPTOR_TYPE_MAX_ENUM ) ) );
}
										...
										const ovrVkProgramParm * parms, const int numParms )
{
	...
	for ( int i = 0; i < numParms; i++ )
	{
					...
					parms[i].type == OVR_PROGRAM_PARM_TYPE_BUFFER_UNIFORM )
		{
			for ( int stageIndex = 0; stageIndex < OVR_PROGRAM_STAGE_MAX; stageIndex++ )
			{
				...
				{
					numSampledTextureBindings[stageIndex] += ( parms[i].type == OVR_PROGRAM_PARM_TYPE_TEXTURE_SAMPLED );
					numStorageTextureBindings[stageIndex] += ( parms[i].type == OVR_PROGRAM_PARM_TYPE_TEXTURE_STORAGE );
					numUniformBufferBindings[stageIndex] += ( parms[i].type == OVR_PROGRAM_PARM_TYPE_BUFFER_UNIFORM );
				}
			}
	...
	for ( int push0 = 0; push0 < layout->numPushConstants; push0++ )
	{
		...
		{
			assert( layout->pushConstants[push0]->binding >= layout->pushConstants[push1]->binding + ovrVkProgramParm_GetPushConstantSize( layout->pushConstants[push1]->type ) ||
					layout->pushConstants[push0]->binding + ovrVkProgramParm_GetPushConstantSize( layout->pushConstants[push0]->type ) <= layout->pushConstants[push1]->binding );
		}
	}
...

	{
		...
		for ( int i = 0; i < numParms; i++ )
		{
			if ( ovrGpuProgramParm_IsOpaqueBinding( parms[i].type ) )
			{
				...
				descriptorSetBindings[numDescriptorSetBindings].descriptorType = ovrGpuProgramParm_GetDescriptorType( parms[i].type );
				descriptorSetBindings[numDescriptorSetBindings].descriptorCount = 1;
				descriptorSetBindings[numDescriptorSetBindings].stageFlags = ovrGpuProgramParm_GetShaderStageFlags( parms[i].stageFlags );
				descriptorSetBindings[numDescriptorSetBindings].pImmutableSamplers = NULL;
				numDescriptorSetBindings++;
			}
			else // push constant
			{
				pushConstantRanges[numPushConstantRanges].stageFlags = ovrGpuProgramParm_GetShaderStageFlags( parms[i].stageFlags );
				pushConstantRanges[numPushConstantRanges].offset = parms[i].binding;
				pushConstantRanges[numPushConstantRanges].size = ovrVkProgramParm_GetPushConstantSize( parms[i].type );
										...
										const ovrVkVertexAttribute * vertexLayout, const int vertexAttribsFlags )
{
	...
	ovrGpuDevice_CreateShader( context->device, &program->vertexShaderModule, VK_SHADER_STAGE_VERTEX_BIT, vertexSourceData, vertexSourceSize );
	ovrGpuDevice_CreateShader( context->device, &program->fragmentShaderModule, VK_SHADER_STAGE_FRAGMENT_BIT, fragmentSourceData, fragmentSourceSize );
...
void InitVertexAttributes( const bool instance,
								const ovrVkVertexAttribute * vertexLayout, const int numAttribs,
...
{
	ovrVkProgramParmState_SetParm( &command->parmState, &command->pipeline->program->parmLayout, index, OVR_PROGRAM_PARM_TYPE_BUFFER_UNIFORM, buffer );
}
										...
										const ovrVkProgramParmState * parms )
{
...

	{
		...
		{
			VkDescriptorType type = ovrGpuProgramParm_GetDescriptorType( parmLayout->bindings[i]->type );
...

	{
		...
		for ( int i = 0; i < parmLayout->numBindings; i++ )
		{
			...
			writes[numWrites].descriptorType = ovrGpuProgramParm_GetDescriptorType( parmLayout->bindings[i]->type );
...
void ovrVkCommandBuffer_Create( ovrVkContext * context, ovrVkCommandBuffer * commandBuffer, const ovrVkCommandBufferType type, const int numBuffers )
{
	...
	commandBuffer->pipelineResources = (ovrVkPipelineResources **) malloc( numBuffers * sizeof( ovrVkPipelineResources * ) );
...
void ovrVkCommandBuffer_Destroy( ovrVkContext * context, ovrVkCommandBuffer * commandBuffer )
{
	...
	for ( int i = 0; i < commandBuffer->numBuffers; i++ )
	{
		...
		for ( ovrVkBuffer * b = commandBuffer->oldMappedBuffers[i], * next = NULL; b != NULL; b = next )
		{
			next = b->next;
			ovrVkBuffer_Destroy( context->device, b );
			free( b );
		...
		for ( ovrVkPipelineResources * r = commandBuffer->pipelineResources[i], * next = NULL; r != NULL; r = next )
		{
			next = r->next;
...
void ovrVkCommandBuffer_ManageBuffers( ovrVkCommandBuffer * commandBuffer )
{
...

	{
		...
		for ( ovrVkBuffer ** b = &commandBuffer->oldMappedBuffers[commandBuffer->currentBuffer]; *b != NULL; )
		{
		...
		for ( ovrVkBuffer * b = commandBuffer->mappedBuffers[commandBuffer->currentBuffer], * next = NULL; b != NULL; b = next )
		{
			next = b->next;
...

	{
		...
		for ( ovrVkPipelineResources ** r = &commandBuffer->pipelineResources[commandBuffer->currentBuffer]; *r != NULL; )
		{
...
{
	ovrVkTexture_ChangeUsage( commandBuffer->context, commandBuffer->cmdBuffers[commandBuffer->currentBuffer], texture, usage );
}
...
void ovrVkCommandBuffer_BeginFramebuffer( ovrVkCommandBuffer * commandBuffer, ovrVkFramebuffer * framebuffer, const int arrayLayer, const ovrVkTextureUsage usage )
{
	...
	ovrVkCommandBuffer_ChangeTextureUsage( commandBuffer, &framebuffer->colorTextures[framebuffer->currentBuffer], usage );
...
void ovrVkCommandBuffer_EndFramebuffer( ovrVkCommandBuffer * commandBuffer, ovrVkFramebuffer * framebuffer, const int arrayLayer, const ovrVkTextureUsage usage )
{
	...
	{
		ovrVkCommandBuffer_ChangeTextureUsage( commandBuffer, &framebuffer->renderTexture, OVR_TEXTURE_USAGE_TRANSFER_SRC );
		ovrVkCommandBuffer_ChangeTextureUsage( commandBuffer, &framebuffer->colorTextures[framebuffer->currentBuffer], OVR_TEXTURE_USAGE_TRANSFER_DST );
		...
		ovrVkCommandBuffer_ChangeTextureUsage( commandBuffer, &framebuffer->renderTexture, OVR_TEXTURE_USAGE_COLOR_ATTACHMENT );
	}
#endif
												...
												VkPipelineBindPoint bindPoint )
{
	...
	const bool descriptorsMatch = ovrVkProgramParmState_DescriptorsMatch( newLayout, newParmState, oldLayout, oldParmState );
	if ( !descriptorsMatch )
	{
		// Try to find existing resources that match.
		ovrVkPipelineResources * resources = NULL;
		for ( ovrVkPipelineResources * r = commandBuffer->pipelineResources[commandBuffer->currentBuffer]; r != NULL; r = r->next )
		{
			if ( ovrVkProgramParmState_DescriptorsMatch( newLayout, newParmState, r->parmLayout, &r->parms ) )
			{
				r->unusedCount = 0;
				resources = r;
				break;
		...
		if ( resources == NULL )
		{
			...
			ovrVkPipelineResources_Create( commandBuffer->context, resources, newLayout, newParmState );
	...
	{
		const void * data = ovrVkProgramParmState_NewPushConstantData( newLayout, i, newParmState, oldLayout, i, oldParmState, false );
		if ( data != NULL )
		{
			const ovrVkProgramParm * newParm = newLayout->pushConstants[i];
			const VkShaderStageFlags stageFlags = ovrGpuProgramParm_GetShaderStageFlags( newParm->stageFlags );
...
void ovrVkCommandBuffer_SubmitGraphicsCommand( ovrVkCommandBuffer * commandBuffer, const ovrVkGraphicsCommand * command )
{
	...
	const ovrVkProgramParmLayout * stateLayout = ( state->pipeline != NULL ) ? &state->pipeline->program->parmLayout : NULL;

	ovrVkCommandBuffer_UpdateProgramParms( commandBuffer, commandLayout, stateLayout, &command->parmState, &state->parmState, VK_PIPELINE_BIND_POINT_GRAPHICS );
	...
	if ( state->pipeline == NULL || geometry != state->pipeline->geometry || command->vertexBuffer != state->vertexBuffer || command->instanceBuffer != state->instanceBuffer )
	{
		...
		const VkIndexType indexType = ( sizeof( ovrTriangleIndex ) == sizeof( unsigned int ) ) ? VK_INDEX_TYPE_UINT32 : VK_INDEX_TYPE_UINT16;
...
ovrVkBuffer * ovrVkCommandBuffer_MapBuffer( ovrVkCommandBuffer * commandBuffer, ovrVkBuffer * buffer, void ** data )
{
	...
	for ( ovrVkBuffer ** b = &commandBuffer->oldMappedBuffers[commandBuffer->currentBuffer]; *b != NULL; b = &(*b)->next )
	{
	...
	if ( newBuffer == NULL )
	{
		...
		ovrVkBuffer_Create( commandBuffer->context, newBuffer, buffer->type, buffer->size, NULL, true );
	}
...
{
	void * data = NULL;
	ovrVkBuffer * buffer = ovrVkCommandBuffer_MapBuffer( commandBuffer, &geometry->instanceBuffer, &data );

	attribs->layout = geometry->layout;
	ovrVkVertexAttributeArrays_Map( attribs, data, buffer->size, geometry->instanceCount, geometry->instanceAttribsFlags );
...
{
	ovrVkCommandBuffer_UnmapBuffer( commandBuffer, &geometry->instanceBuffer, mappedInstanceBuffer, type );
}
