...
namespace OVR
{
	...
	public:
		T Filter(T x, float dt) { return x; }
	};
	...
	public:
		...
		{
			if (!warm) { warm = true; return last = x; }
		...
		constexpr T Last() const { return last; }
	...
	public:
		...
		T Filter(T x, float dt)
		{
			...
			if (!warm) { warm = true; last = x; return T{}; }
	...
	public:
		...
		T Filter(T x, float dt)
		{
			...
			velocity = OVRF::lerp(velocity, OVRF::scale(OVRF::diff(next, last), params.vscale), OVRF::cuttoffToGain(params.vcutoff, dt));
			return last = next;
		}

		constexpr T Last() const { return last; }
		constexpr T Velocity() const { return velocity; }
	...
	public:
		...
		OneEuroFilter(OneEuroParams p) :  params(p), warm(false), last(T{}), xfilter(p.cutoff), dxfilter(p.vcutoff) {}
		...
		T Filter(T x, float dt)
		{
			if ( dt > 0.0f )
			{
				if (!warm)
				{
					...
					// dxfilter must wait until we have another sample to compute intantanious velocity from
				...
				//if (params.relax == 5.1f) ALOG("VrHands effective cutoff: %.3f speed: %.3f", xfilter.params.cutoff, filteredSpeed);
				return last = xfilter.Filter(x, dt);
			}
			else return Last();
		}
	...
	public:
		...
		{
			if (hold && !Holding) HeldValue = Value; // hold the previous value
			Holding = hold;
