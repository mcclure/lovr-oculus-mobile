...
public:
	...
	inline _storageType_ ToBit( const _enumType_ e ) { return static_cast<_storageType_>( 1 ) << e; }

	BitFlagsT( const _enumType_ e ) :
		Value( ToBit( e ) )
	{
	}

	BitFlagsT( const BitFlagsT & other ) :
		Value( other.Value )
	{
	}

	BitFlagsT( const eAllBits allBits ) :
		Value( AllBits() )
	{
	}
	...
	_storageType_	GetValue() const { return Value; }
...
private:
	...
	// significantly.
	explicit BitFlagsT( const _storageType_ value ) :
		Value( value )
	{
	}
};
