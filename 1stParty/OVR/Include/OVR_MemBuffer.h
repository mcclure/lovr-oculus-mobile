...
public:
	...
	explicit MemBufferFile(
		eNoInit const
			noInit )	// use this constructor as MemBufferFile( NoInit ) -- this takes a parameters so you still can't defalt-construct by accident
	{
		OVR_UNUSED( noInit );
	}
...
public:
	...
	explicit MemBufferT( void *& buffer, size_t const size ) : Buffer( static_cast<C *>( buffer ) ), Size( size )
	{
		buffer = nullptr;
	}
