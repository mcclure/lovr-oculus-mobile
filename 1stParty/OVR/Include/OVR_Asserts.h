...
#if defined(OVR_ASSERT_CLIENT) || defined(OVR_ASSERT_SERVER) || defined(OVR_ASSERT_GUARDIAN)
...
// Sends Guardian assert logs.
void SendAssertLogsGuardian( const char* filePath, const char* funcName, const unsigned int lineNo, const char* condition );
#endif
#endif
...
//--------------------------------------------------------------------------------------------------
	...
	#define OVR_DEBUG_BREAK  ((void)0)
		...
		#elif defined(OVR_ASSERT_GUARDIAN)
			#define OVR_ASSERT(p) \
				if (!(p)) { \
					SendAssertLogsGuardian( __FILE__, __FUNCTION__, __LINE__, #p ); }
		#endif
	#else
		#define OVR_DEBUG_BREAK  ((void)0)
