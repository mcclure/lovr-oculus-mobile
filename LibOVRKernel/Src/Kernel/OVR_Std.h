...
char * OVR_CDECL OVR_strdup( const char * str );
...
#endif
	// Windows: for vsnprintf and vsnprintf_s with _TRUNCATE, a return of -1 means the buffer was overflowed
	// Android / Linux: -1 is only returned if a format error occured, bytesWritten == destsize means buffer overflow
